...
void CachedImage::data ( QBuffer &_buffer, bool eof )
{
    ...
    if ( translator == NULL ) {
	os::TranslatorFactory* pcFactory = os::TranslatorFactory::GetDefaultFactory();

	int nError = pcFactory->FindTranslator( "", os::TRANSLATOR_TYPE_IMAGE,
						(const void*)_buffer.buffer().data(), _buffer.size(), &translator );
