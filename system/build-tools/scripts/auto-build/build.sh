...
build update 2>>$LOG 1>>$LOG
sync

echo "Updating sources"
cd $SOURCE_DIR/syllable
cvs -z9 update -dP 2>>$LOG 1>>$LOG
sync
...
build install $HOME/Packages/gcc-3.4.3.bin.1.zip 2>>$LOG 1>>$LOG
sync
echo "Building GCC 4.1.1"
image gcc-libraries 2>>$LOG 1>>$LOG
build log >> $SYSTEM_LOG
build log failures >> $SYSTEM_FAILURE_LOG
build log summary >> $SYSTEM_SUMMARY_LOG
sync
...
build install $HOME/Packages/gcc-4.1.1.bin.1.zip 2>>$LOG 1>>$LOG
sync

# XXXKV: We have to build Xpdf with Freetype-2.1.10..
echo "Switching to Freetype-2.1.10"
build install $HOME/Packages/freetype-2.1.10.bin.1.zip 2>>$LOG 1>>$LOG
sync
echo "Building Xpdf"
image xpdf 2>>$LOG 1>>$LOG
build log >> $SYSTEM_LOG
build log failures >> $SYSTEM_FAILURE_LOG
build log summary >> $SYSTEM_SUMMARY_LOG
sync
echo "Switching to Freetype-2.2.1"
build install $HOME/Packages/freetype-2.2.1.bin.1.zip 2>>$LOG 1>>$LOG
sync

# XXXKV: Now on with the show...
cd stage/image
build scrub 2>>$LOG 1>>$LOG
