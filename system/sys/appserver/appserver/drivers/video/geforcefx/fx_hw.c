...
void NVLockUnlock( NVPtr pNv, Bool Lock )
{
	CARD8 cr11;
...
int NVShowHideCursor( NVPtr pNv, int ShowHide )
{
...
static void nv10UpdateArbitrationSettings( unsigned VClk, unsigned pixelDepth, unsigned *burst, unsigned *lwm, NVPtr pNv )
{
...
static void nForceUpdateArbitrationSettings( unsigned VClk, unsigned pixelDepth, unsigned *burst, unsigned *lwm, NVPtr pNv )
{
	nv10_fifo_info fifo_data;
	nv10_sim_state sim_data;
	...
	if ( ( pNv->Chipset & 0x0FF0 ) == 0x01A0 )
	{
		...
		uMClkPostDiv = ( pci_gfx_read_config( pNv->Fd, 0, 0, 3, 0x6C, 4 ) >> 8 ) & 0xf;
	...
	{
		MClk = pci_gfx_read_config( pNv->Fd, 0, 0, 5, 0x4C, 4 ) / 1000;
	}
	...
	sim_data.enable_mp = 0;
	sim_data.memory_type = ( pci_gfx_read_config( pNv->Fd, 0, 0, 1, 0x7C, 4 ) >> 12 ) & 1;
	sim_data.memory_width = 64;

	memctrl = pci_gfx_read_config( pNv->Fd, 0, 0, 3, 0x00, 4 ) >> 16;

	if ( ( memctrl == 0x1A9 ) || ( memctrl == 0x1AB ) || ( memctrl == 0x1ED ) )
	{
		int dimm[3];

		dimm[0] = ( pci_gfx_read_config( pNv->Fd, 0, 0, 2, 0x40, 4 ) >> 8 ) & 0x4F;
		dimm[1] = ( pci_gfx_read_config( pNv->Fd, 0, 0, 2, 0x44, 4 ) >> 8 ) & 0x4F;
		dimm[2] = ( pci_gfx_read_config( pNv->Fd, 0, 0, 2, 0x44, 4 ) >> 8 ) & 0x4F;

		if ( ( dimm[0] + dimm[1] ) != dimm[2] )
		{
			dbprintf( "WARNING: " "your nForce DIMMs are not arranged in optimal banks!\n" );
		}
	}
...
static void CalcVClock2Stage( int clockIn, int *clockOut, U032 *pllOut, U032 *pllBOut, NVPtr pNv )
{
	unsigned DeltaNew, DeltaOld;
	unsigned VClk, Freq;
	unsigned M, N, P;
 ...
 */
void NVCalcStateExt( NVPtr pNv, FX_HW_STATE * state, int bpp, int width, int hDisplaySize, int height, int dotClock, int flags )
{
	int pixelDepth, VClk;
	...
	default:
		...
		if ( ( ( pNv->Chipset & 0xffff ) == 0x01A0 ) || ( ( pNv->Chipset & 0xffff ) == 0x01f0 ) )
		{
...
void NVUnloadStateExt( NVPtr pNv, FX_HW_STATE * state )
{
	VGA_WR08( pNv->PCIO, 0x03D4, 0x19 );
