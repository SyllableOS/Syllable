                  ...
                  PSIS_HW_DEVICE_INFO HwDeviceExtension,USHORT RefreshRateTableIndex)
{
  ...
  /*     650/LV and 650/302LV BIOS do not check this either; so we set it... */
  ...
  temp = (tempax & 0xFF00) >> 8;
  temp = (temp >> 1) & 0x09;
  /* TW: Inserted from 630/301B and 650/30xLV BIOS; not in 630/301 BIOS */
  if(!(SiS_Pr->SiS_VBType & VB_SIS301)) {
       temp |= 0x01;
  }
                   ...
                   PSIS_HW_DEVICE_INFO HwDeviceExtension,USHORT RefreshRateTableIndex)
{
  ...
  /* TW: Inserted from 650/30xLV+LVDS BIOSes */
  if(SiS_Pr->SiS_LCDResInfo == SiS_Pr->SiS_Panel640x480) {
    tempbx = SiS_Pr->SiS_VGAVDE;
    tempcx = tempbx;
    tempbx--;
                    ...
                    PSIS_HW_DEVICE_INFO HwDeviceExtension)
{
  ...
  /* TW: 650/LVDS (1.10.07, 1.10.00), 650/301LV, 330 overrule calculated value; 315 does not */
                    ...
                    PSIS_HW_DEVICE_INFO HwDeviceExtension)
{
  ...
  if(SiS_Pr->SiS_VBInfo & SetCRT2ToLCDA) {
	...
	/* TW: Inserted from 630/30xLV BIOS 1.10.6s */
	if(HwDeviceExtension->jChipType >= SIS_315H) {
	    if(SiS_Pr->SiS_VBType & VB_SIS301BLV302BLV) {
...
/* TW: Checked against 650+301, 650/LVDS (1.10.07) and 650/30xLV (1.10.6s) BIOS */
void
  ...
  if(HwDeviceExtension->jChipType >= SIS_315H) {
     ...
     if(SiS_Pr->SiS_IF_DEF_LVDS == 0) {
		 ...
		 if(ModeNo == 0x3a || ModeNo == 0x4d || ModeNo == 0x65) {
		     /* 1400x1050 panels can't scale to 1280x1024, 
		      * so we do it outselves 
		      */
...
/*       if((SiS_Pr->SiS_VBInfo & (SetCRT2ToTV | SetPALTV)) && (!(SiS_Pr->SiS_HiVision & 0x03))) {  */
	 ...
	 if(HwDeviceExtension->jChipType > SIS_315H) {
	     if(SiS_Pr->SiS_VBType & VB_SIS302LV) {
                temp = 0x0026;  /* From 1.10.7w, 1.10.8r; 1.10.6s: 0x0036 */
	     } else {
	        temp = 0x0036;
	     }
	 } else {
                 ...
                 USHORT RefreshRateTableIndex,PSIS_HW_DEVICE_INFO HwDeviceExtension)
{
  ...
  if(SiS_Pr->SiS_VBType & VB_SIS301BLV302BLV) {
	...
	/* TW: New from 650/301LV, 302LV BIOS (300/301LV does not do this) */
	if(HwDeviceExtension->jChipType >= SIS_315H) {
	  if(SiS_Pr->SiS_VBType & VB_SIS301BLV302BLV) {
            if(SiS_Pr->SiS_VBInfo & SetCRT2ToTV) {
	      if(!(SiS_Pr->SiS_VBInfo & SetPALTV)) {
		...
		BOOLEAN checkcr32)
{
	...
	SiS_Pr->SiS_DDC_Data = 0x02 << temp;
        ...
        if(pSiS->VBFlags & (VB_301LV|VB_302LV)) myadaptnum = 0;
...
/* For 310/325/330 series + 30xB bridge only */
USHORT
SiS_SenseLCDDDC(SiS_Private *SiS_Pr, SISPtr pSiS)
{
   ...
   if(pSiS->VGAEngine != SIS_315_VGA) return 0;
   if(!(pSiS->VBFlags & (VB_301B|VB_302B))) return 0;
   
   /* Normal or DH version? (DH version only supports VGA2/TV) */
   if(!(SiS_GetReg1(SiS_Pr->SiS_Part4Port,0x23) & 0x02)) {
        xf86DrvMsg(pSiS->pScrn->scrnIndex, X_PROBED, 
        	"This version of SiS30xB does not support LCD output\n");
   	return 0;
   }	
   ...
   case 1:							/* Analyze EDID V1 */
	 ...
	 case 1280:
		...
		cr37 |= 0x10;			/* Bug in timing somewhere, we (=bridge) always scale */
	     } else if(yres == 1024) {
             ...
             UCHAR *ROMAddr, USHORT ModeNo)
{
  ...
  if(SiS_Pr->SiS_VBInfo & SetCRT2ToRAMDAC) {			/* VGA */
     ...
     if((IS_SIS650740) && (SiS_Pr->SiS_VBType & VB_SIS301LV302LV)) { 	/* 650+301LV */
       if(SiS_IsM650or651(SiS_Pr,HwDeviceExtension, BaseAddr)) {
          if((ROMAddr) && SiS_Pr->SiS_UseROM) {
#if 0	     /* Always use the second pointer on 650; some BIOSes */
             /* still carry 301 data at the first location        */  
	  ...
	  if(romptr) delay = ROMAddr[(romptr + index)];
             ...
             delay = SiS310_LCDDelayCompensation_650301B[index];    /* 652c */
#if 0	     
	     if(SiS_Pr->SiS_VBType & VB_SIS302LV)
	        delay = SiS310_LCDDelayCompensation_650301B[index]; /* 6541 */
#endif		
	  }
       } else {
          delay = SiS310_LCDDelayCompensation_651301LV[index];      /* 6550 */
	  if(SiS_Pr->SiS_VBType & VB_SIS302LV)
	     delay = SiS310_LCDDelayCompensation_651302LV[index];   /* 655f */
       }
     } else {
        if((ROMAddr) && SiS_Pr->SiS_UseROM) {				/* 315, 330, 650+301B */
           if(HwDeviceExtension->jChipType < SIS_330) {
              romptr = ROMAddr[0x120] | (ROMAddr[0x121] << 8);  
	      if(SiS_Pr->SiS_VBType & VB_SIS301BLV302BLV)
	         romptr = ROMAddr[0x122] | (ROMAddr[0x123] << 8);
        ...
        if(romptr) delay = ROMAddr[(romptr + index)];
           ...
           delay = SiS310_LCDDelayCompensation_301[index];
           if(SiS_Pr->SiS_VBType & VB_SIS301BLV302BLV) {
	      if(IS_SIS650740) {
	         delay = SiS310_LCDDelayCompensation_650301B[index];
	      } else {
                 delay = SiS310_LCDDelayCompensation_3xx301B[index];
	      }
	   }
           if(SiS_Pr->SiS_IF_DEF_LVDS == 1)
              delay = SiS310_LCDDelayCompensation_LVDS[index];
        }
     }
     ...
     if((IS_SIS650740) && (SiS_Pr->SiS_VBType & VB_SIS301LV302LV)) {
       if(SiS_IsM650or651(SiS_Pr,HwDeviceExtension, BaseAddr)) {
          if((ROMAddr) && SiS_Pr->SiS_UseROM) {
#if 0	     /* Always use the second pointer on 650; some BIOSes */
             /* still carry 301 data at the first location        */  	  
	  ...
	  if(romptr) delay = ROMAddr[romptr + index];
	     ...
	     delay = SiS310_TVDelayCompensation_301B[index];      /* 656e @ 0x114 */
#if 0	     
	     if(SiS_Pr->SiS_VBType & VB_SIS302LV)
	        delay = SiS310_TVDelayCompensation_301B[index];   /* 6574 @ 0x11a */
#endif		
	  }
       } else {
          delay = SiS310_TVDelayCompensation_651301LV[index];        /* 657a */
	  if(SiS_Pr->SiS_VBType & VB_SIS302LV)
	      delay = SiS310_TVDelayCompensation_651302LV[index];   /* 6580 */
       }
     } else {
       if((ROMAddr) && SiS_Pr->SiS_UseROM) {
          if(HwDeviceExtension->jChipType < SIS_330) {
