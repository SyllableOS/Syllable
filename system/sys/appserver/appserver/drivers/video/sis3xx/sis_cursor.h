...
#define sis300GetCursorStatus \
  MMIO_IN32(si.regs, CS(0)) & 0x40000000;
  
#define sis300SetCursorStatus(status) \
  { \
  unsigned long temp; \
  temp = MMIO_IN32(si.regs, CS(0)); \
  temp &= 0xbfffffff; \
  temp |= status; \
  MMIO_OUT32(si.regs, CS(0), temp); \
  }
...
#define sis300SetCursorBGColor(color)\
  MMIO_OUT32(si.regs, CS(1), (color));
#define sis300SetCursorFGColor(color)\
  MMIO_OUT32(si.regs, CS(2), (color));

#define sis300SetCursorPositionX(x,preset)\
  MMIO_OUT32(si.regs, CS(3), ((x) | ((preset) << 16)));
#define sis300SetCursorPositionY(y,preset)\
  MMIO_OUT32(si.regs, CS(4), ((y) | ((preset) << 16)));
...
#define sis301GetCursorStatus \
  MMIO_IN32(si.regs, CS(8)) & 0x40000000;
  
#define sis301SetCursorStatus(status) \
  { \
  unsigned long temp; \
  temp = MMIO_IN32(si.regs, CS(8)); \
  temp &= 0xbfffffff; \
  temp |= status; \
  MMIO_OUT32(si.regs, CS(8), temp); \
  }
...
#define sis301SetCursorBGColor(color)\
  MMIO_OUT32(si.regs, CS(9), (color));
#define sis301SetCursorFGColor(color)\
  MMIO_OUT32(si.regs, CS(10), (color));

#define sis301SetCursorPositionX(x,preset)\
  MMIO_OUT32(si.regs, CS(11), ((x) | ((preset) << 16)));
#define sis301SetCursorPositionY(y,preset)\
  MMIO_OUT32(si.regs, CS(12), ((y) | ((preset) << 16)));
...
#define sis301SetCursorAddress310(address)\
  { \
  unsigned long temp; \
  if(sishw_ext.jChipType == SIS_315H) { \
     if(address & 0x10000) { \
        address &= ~0x10000; \
