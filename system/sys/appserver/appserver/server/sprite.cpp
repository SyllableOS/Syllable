...
void SrvSprite::ColorSpaceChanged()
{
	...
	if ( pcSprite->m_pcImage != NULL ) {
	    ...
	    pcSprite->m_pcBackground = new SrvBitmap( pcSprite->m_pcImage->m_nWidth, pcSprite->m_pcImage->m_nHeight,
						      pcSprite->m_pcTarget->m_eColorSpc );
	}
    }
}
...
void SrvSprite::Draw()
{
	...
	if ( cClipRect.IsValid() ) {
	    ...
	    pcDriver->BltBitmap( m_pcTarget, m_pcImage, cClipRect.Bounds() + cOffset,
				 m_cPosition - m_cHotSpot + cOffset, DM_BLEND );
	}
    } else {
	IRect cFrame = m_cBounds + m_cPosition - m_cHotSpot;
	pcDriver->DrawLine( m_pcTarget, cBitmapRect,
			    IPoint( cFrame.left, cFrame.top ), IPoint( cFrame.right, cFrame.top ),
			    Color32_s(), DM_INVERT );
	pcDriver->DrawLine( m_pcTarget, cBitmapRect,
			    IPoint( cFrame.right, cFrame.top ), IPoint( cFrame.right, cFrame.bottom ),
			    Color32_s(), DM_INVERT );
	pcDriver->DrawLine( m_pcTarget, cBitmapRect,
			    IPoint( cFrame.right, cFrame.bottom ), IPoint( cFrame.left, cFrame.bottom ),
			    Color32_s(), DM_INVERT );
	pcDriver->DrawLine( m_pcTarget, cBitmapRect,
			    IPoint( cFrame.left, cFrame.bottom ), IPoint( cFrame.left, cFrame.top ),
			    Color32_s(), DM_INVERT );
    }
}
...
void SrvSprite::Erase()
{
	...
	if ( cClipRect.IsValid() ) {
	    pcDriver->BltBitmap( m_pcTarget, m_pcBackground, cClipRect.Bounds() + cOffset,
				 m_cPosition - m_cHotSpot + cOffset, DM_COPY );
	}
    } else {
	IRect cFrame = m_cBounds + m_cPosition - m_cHotSpot;
	pcDriver->DrawLine( m_pcTarget, cBitmapRect,
...
void SrvSprite::Draw( SrvBitmap* pcTarget, const IPoint& cPos )
{
	...
	if ( cClipRect.IsValid() ) {
	    ...
	    pcDriver->BltBitmap( pcTarget, m_pcImage, cClipRect.Bounds() + cOffset,
				 cPos + cOffset, DM_BLEND );
	}
    } else {
	IRect cFrame = m_cBounds + cPos;
	pcDriver->DrawLine( pcTarget, cBitmapRect,
			    IPoint( cFrame.left, cFrame.top ), IPoint( cFrame.right, cFrame.top ),
			    Color32_s(), DM_INVERT );
	pcDriver->DrawLine( pcTarget, cBitmapRect,
			    IPoint( cFrame.right, cFrame.top ), IPoint( cFrame.right, cFrame.bottom ),
