...
void WinSelect::UpdateWinList( bool bMoveToFront, bool bSetFocus )
{
	...
	if ( bMoveToFront ) {
		...
		if ( pcWindow == pcLayer->GetWindow() ) {
		    ...
		    if ( pcWindow->GetFrame( false ).DoIntersect( Rect( 0, 0, g_pcScreenBitmap->m_nWidth - 1,
									g_pcScreenBitmap->m_nHeight - 1 ) ) == false )
		    {
			Rect cFrame = pcWindow->GetFrame();
			cFrame = Rect( 0, 0, cFrame.Width(), cFrame.Height() ) +
			    Point( g_pcScreenBitmap->m_nWidth / 2 - (cFrame.Width()+1.0f) / 2,
				    g_pcScreenBitmap->m_nHeight / 2 - (cFrame.Height()+1.0f) / 2 );
