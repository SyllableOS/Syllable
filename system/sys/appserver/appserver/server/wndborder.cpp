...
static inline Rect RectToClient( const Rect& cRect, WindowDecorator* pcDecorator )
{
    ...
    return( Rect( cRect.left   + cBorders.left,
...
WndBorder::WndBorder( SrvWindow* pcWindow, Layer* pcParent, const char* pzName, bool bBackdrop )
    : Layer( pcWindow, pcParent, pzName, Rect(0,0,0,0), 0, NULL ), m_cMinSize(0.0f,0.0f),m_cMaxSize(INT_MAX,INT_MAX),
...
bool WndBorder::MouseMoved( Messenger* pcAppTarget, const Point& cNewPos, int nTransit )
{
	...
	if ( eCursorHitItem != m_eCursorHitItem ) {
		...
		case WindowDecorator::HIT_SIZE_LEFT:
		case WindowDecorator::HIT_SIZE_RIGHT:
		    SrvApplication::SetCursor( MPTR_MONO, IPoint(H_POINTER_WIDTH/2,H_POINTER_HEIGHT/2),
					       g_anHMouseImg, H_POINTER_WIDTH, H_POINTER_HEIGHT );
		    break;
		case WindowDecorator::HIT_SIZE_TOP:
		case WindowDecorator::HIT_SIZE_BOTTOM:
		    SrvApplication::SetCursor( MPTR_MONO, IPoint(V_POINTER_WIDTH/2,V_POINTER_HEIGHT/2),
					       g_anVMouseImg, V_POINTER_WIDTH, V_POINTER_HEIGHT );
		    break;
		case WindowDecorator::HIT_SIZE_LT:
		case WindowDecorator::HIT_SIZE_RB:
		    SrvApplication::SetCursor( MPTR_MONO, IPoint(DIAG_POINTER_WIDTH/2,DIAG_POINTER_HEIGHT/2),
					       g_anLTMouseImg, DIAG_POINTER_WIDTH, DIAG_POINTER_HEIGHT );
		    break;
		case WindowDecorator::HIT_SIZE_LB:
		case WindowDecorator::HIT_SIZE_RT:
		    SrvApplication::SetCursor( MPTR_MONO, IPoint(DIAG_POINTER_WIDTH/2,DIAG_POINTER_HEIGHT/2),
					       g_anLBMouseImg, DIAG_POINTER_WIDTH, DIAG_POINTER_HEIGHT );
		    break;
...
bool WndBorder::MouseDown( Messenger* pcAppTarget, const Point& cPos, int nButton )
{
	...
	m_nCloseDown = 1;
	m_pcDecorator->SetCloseButtonState( true );
    } else if ( m_eHitItem == WindowDecorator::HIT_ZOOM ) {
	m_nZoomDown = 1;
	m_pcDecorator->SetZoomButtonState( true );
    } else if ( m_eHitItem == WindowDecorator::HIT_DEPTH ) {
	m_nToggleDown = 1;
	m_pcDecorator->SetDepthButtonState( true );
    } else if ( m_eHitItem == WindowDecorator::HIT_DRAG ) {
	SrvApplication::SetCursor( MPTR_MONO, IPoint(0,0), NULL, 0, 0 );
    }
    return( m_eHitItem != WindowDecorator::HIT_NONE );
...
void WndBorder::MouseUp( Messenger* pcAppTarget, const Point& cPos, int nButton )
{
    ...
    if ( m_nToggleDown == 1 ) {
	m_pcDecorator->SetDepthButtonState( false );
    }
    if ( m_nZoomDown == 1 ) {
	m_pcDecorator->SetZoomButtonState( false );
    }
    if ( m_nCloseDown == 1 ) {
	m_pcDecorator->SetCloseButtonState( false );
    }
