...
File::File( const std::string& cPath, int nOpenMode ) :
	FSNode( cPath, nOpenMode & ~O_NOTRAVERSE ), m_cMutex( "os::File" )
{
...
File::File( const Directory& cDir, const std::string& cPath, int nOpenMode ) :
	FSNode( cDir, cPath, nOpenMode & ~O_NOTRAVERSE ), m_cMutex( "os::File" )
{
    if ( S_ISDIR( GetMode( false ) ) ) {
	throw errno_exception( "Node not a file", EINVAL );
    }
...
File::File( const FileReference& cRef, int nOpenMode ) :
	FSNode( cRef, nOpenMode & ~O_NOTRAVERSE ), m_cMutex( "os::File" )
{
    if ( S_ISDIR( GetMode( false ) ) ) {
	throw errno_exception( "Node not a file", EINVAL );
    }
