...
status_t Settings::SetData( const char* pzName, int nType, const void* pData, uint32 nSize, int nIndex = 0, bool bFixedSize = true, int nMaxCountHint = 1 )
{
    ...
    if ( psArray == NULL || nIndex >= psArray->nCount ) {
    	...
    	if( bFixedSize ) {
	    	memcpy( (uint8*)psArray + sizeof(DataArray_s) + psArray->nNameLength + nIndex*psArray->nChunkSize,
	    		pData, nSize );
	    	return 0;
	} else {
		Chunk_s* psChunk;
...
Message::Chunk_s* Settings::_ResizeChunk( Message::DataArray_s* psArray, Message::Chunk_s* psChunk, uint32 nSize )
{
	...
	if ( psNewArray != NULL )
	{
	    ...
	    if( nSizeAfterChunk > 0 ) {
		    memcpy( ((uint8*)psNewArray + nNewChunkEnd),
		    	((uint8*)psChunk + psChunk->nDataSize + 4),
		    	nSizeAfterChunk );
	    }
