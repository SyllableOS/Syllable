...
uint8* Message::_CreateArray( const char* pzName, int nType, const void* pData, uint32 nSize,
			     bool bFixedSize, int nMaxCountHint )
{
    assert( pzName != NULL );
...
status_t Message::AddData( const char* pzName, int nType, const void* pData, uint32 nSize,
			   bool bFixedSize, int nMaxCountHint )
{
    assert( pzName != NULL );
    assert( pData != NULL );
...
status_t Message::FindData( const char* pzName, int nType,
			    const void** ppData, size_t* pnSize, int nIndex ) const
{
    DataArray_s* psArray = _FindArray( pzName, nType );
