...
{
    if( m_iThread < 0 ) throw ThreadException( "Invalid thread ID", EINVALIDTHREAD );
...
{
    if( m_iThread >= 0 ) Terminate();

    m_iThread = spawn_thread( pzName, _Entry, nPriority, 0, this );
    if( m_iThread < 0 ) throw ThreadException( "spawn_thread() failed", errno );
}
