...
static status_t conv_error( status_t nError )
{
	...
	case 0:
	    return( 0 );
	case REG_BADRPT:	return( RegExp::ERR_BADREPEAT );
	case REG_BADBR:		return( RegExp::ERR_BADBACKREF );
	case REG_EBRACE:	return( RegExp::ERR_BADBRACE );
	case REG_EBRACK:	return( RegExp::ERR_BADBRACK );
	case REG_ERANGE:	return( RegExp::ERR_BADRANGE );	
	case REG_ECTYPE:	return( RegExp::ERR_BADCHARCLASS );
	case REG_EPAREN:	return( RegExp::ERR_BADPARENTHESIS );
	case REG_ESUBREG:	return( RegExp::ERR_BADSUBREG );
	case REG_EEND:		return( RegExp::ERR_GENERIC );
	case REG_EESCAPE:	return( RegExp::ERR_BADESCAPE );
	case REG_BADPAT:	return( RegExp::ERR_BADPATTERN );
	case REG_ESIZE:		return( RegExp::ERR_TOLARGE );
	case REG_ESPACE:	return( RegExp::ERR_NOMEM );
	default:
...
bool RegExp::Search( const std::string& cString )
{
	...
	if ( m->m_pasMatches[i].rm_so != -1 ) {
	    m->m_cSubStrings.push_back( std::string( cString.begin() + m->m_pasMatches[i].rm_so,
						     cString.begin() + m->m_pasMatches[i].rm_eo ) );
	} else {
