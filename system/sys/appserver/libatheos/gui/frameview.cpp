...
FrameView::FrameView( const Rect& cFrame, const std::string& cName, const std::string& cLable,
		      uint32 nResizeMask, uint32 nFlags ) : LayoutView( cFrame, cName, NULL, nResizeMask, nFlags ), m_cLabelString( cLable )
{
    m_pcLabelView  = NULL;
...
void FrameView::_CalcStringLabelSize()
{
    ...
    m_cLabelSize = Point( ceil( GetStringWidth( m_cLabelString.c_str(), m_cLabelString.size() ) ),
			  ceil( m_sFontHeight.ascender + m_sFontHeight.descender ) );
}
