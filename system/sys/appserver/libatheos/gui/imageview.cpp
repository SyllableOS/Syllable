 ...
 *****************************************************************************/
ImageView::ImageView( Rect cFrame, const char *pzName, Image *pcImage, ImageMode eMode = DEFAULT,
...
Point ImageView::GetPreferredSize(bool bLargest) const
{
	...
	if( m->m_nMode & FIXED_SIZE ) {
		...
		if( bLargest ) return Point( COORD_MAX, COORD_MAX );
		else return m->m_pcImage->GetSize();
	}
}
