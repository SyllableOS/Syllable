...
View::View( const Rect& cFrame, const std::string& cTitle, uint32 nResizeMask, uint32 nFlags )
	: Handler( cTitle ) //, m_cFrame( cFrame ), m_cTitle( cTitle ), m_nResizeMask( nResizeMask ), m_nFlags( nFlags )
{
    m = new Private;
...
View* View::GetChildAt( const Point& cPos )
{
	...
	if ( !((cPos.x < pcChild->m->m_cFrame.left) ||
	       (cPos.x > pcChild->m->m_cFrame.right) ||
	       (cPos.y < pcChild->m->m_cFrame.top) ||
	       (cPos.y > pcChild->m->m_cFrame.bottom)) )
	{
	    return( pcChild );
	}
    }
...
void View::GetStringWidths( const char* apzStringArray[], const int* anLengthArray,
...
void View::GetStringLengths( const char** apzStringArray, const int* anLengthArray, int nStringCount,
