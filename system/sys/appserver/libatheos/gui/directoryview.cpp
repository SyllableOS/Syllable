...
namespace os_priv {
	...
	CopyFileParams_s( const std::vector<std::string> cDstPaths,
			  const std::vector<std::string> cSrcPaths, const Messenger& cViewTarget ) :
    ...
    {
	DeleteFileParams_s( const std::vector<std::string> cPaths, const Messenger& cViewTarget ) :
	       ...
	       bool* pbReplaceFiles, bool* pbDontOverwrite, ProgressRequester* pcProgress )
{
	...
	while( (psEntry = readdir( pDir )) != NULL ) {
	    ...
	    if ( CopyFile( cDstPath.GetPath(), cSrcPath.GetPath(),
...
DirectoryView::DirectoryView( const Rect& cFrame, const std::string& cPath, uint32 nModeFlags, uint32 nResizeMask, uint32 nViewFlags ) :
    ListView( cFrame, "_list_view", nModeFlags, nResizeMask, nViewFlags ), m_cPath( cPath.c_str() )
{
...
void FileRow::Paint( const Rect& cFrame, View* pcView, uint nColumn,
...
bool FileRow::HitTest( View* pcView, const Rect& cFrame, int nColumn, Point cPos )
{
    ...
    Rect cRect( cFrame.left, nBaseLine - sHeight.ascender - 1.0f,
		cFrame.left + pcView->GetStringWidth( m_cName.c_str() ) + 4.0f, nBaseLine + sHeight.descender + 1.0f );
