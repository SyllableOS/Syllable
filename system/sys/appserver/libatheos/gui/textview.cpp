...
namespace os
{
    ...
    {
    public:
	TextEdit( TextView* pcParent, const Rect& cFrame, const char* pzTitle,
	...
	bool	GetNumeric() const { return( m_bNumeric ); }
	...
	int	GetMaxUndoSize() const { return( m_nMaxUndoSize ); }
	...
	const TextView::buffer_type& GetBuffer() const { return( m_cBuffer ); }
...
//    private:
	...
	struct UndoNode
	{
	    UndoNode( int nMode ) { m_nMode = nMode; }
	    int         m_nMode;
	    IPoint      m_cPos;
	    IPoint	    m_cEndPos;
	    std::string m_cText;
...
TextView::TextView( const Rect& cFrame, const char* pzTitle, const char* pzText, uint32 nResizeMask, uint32 nFlags ) :
	Control( cFrame, pzTitle, "", NULL, nResizeMask, nFlags )
{
...
TextEdit::TextEdit( TextView* pcParent, const Rect& cFrame, const char* pzTitle, uint32 nResizeMask, uint32 nFlags ) :
    View( cFrame, pzTitle, nResizeMask, nFlags ), m_cCsrPos(0,0), m_cPrevCursorPos(0,0),
    m_cPreferredSize( LEFT_BORDER+RIGHT_BORDER, TOP_BORDER+BOTTOM_BORDER ),
    m_cMinPreferredSize(0,0), m_cMaxPreferredSize(0,0), m_sCurFgColor( 0, 0, 0 ), m_sCurBgColor( 255, 255, 255 )
{
    m_pcParent = pcParent;
    m_cBuffer.push_back( "" );
...
Point TextEdit::GetPreferredSize( bool bLargest ) const
{
	...
	if ( m_cMaxPreferredSize != IPoint(0,0) ) {
	    return( Point( ceil( m_cMaxPreferredSize.x * GetStringWidth( "M" ) + LEFT_BORDER + RIGHT_BORDER ),
			   ceil( m_cMaxPreferredSize.y * m_vGlyphHeight + TOP_BORDER + BOTTOM_BORDER ) ) );
	} else {
	    if ( m_bMultiLine ) {
	...
	if ( m_cMinPreferredSize != IPoint(0,0) ) {
	    return( Point( ceil( m_cMinPreferredSize.x * GetStringWidth( "M" ) + LEFT_BORDER + RIGHT_BORDER ),
			   ceil( m_cMinPreferredSize.y * m_vGlyphHeight + TOP_BORDER + BOTTOM_BORDER ) ) );
	} else {
...
void TextEdit::Delete( IPoint cStart, IPoint cEnd, bool bMakeUndo )
{
	...
	if ( nLen > 0 ) {
	    m_nCurrentLength -= String( m_cBuffer[cRegStart.y].begin() + cRegStart.x,
					m_cBuffer[cRegStart.y].begin() + cRegStart.x + nLen ).CountChars();
...
bool TextEdit::HandleMouseMove( const Point& cPosition, int nCode, uint32 nButtons, Message* pcData )
{
	...
	Application::GetInstance()->PushCursor( MPTR_MONO, g_anMouseImg, POINTER_WIDTH, POINTER_HEIGHT,
						IPoint( POINTER_WIDTH / 2, POINTER_HEIGHT / 2 ) );
	m_bIBeamActive = true;
...
//		if ( (m_cBuffer[m_cCsrPos.y][m_cCsrPos.x+nDelta] & 0x80) == 0 || (m_cBuffer[m_cCsrPos.y][m_cCsrPos.x+nDelta] & 0xc0) == 0xc0 ) {
	...
	default:
	    std::string cStr( pzString );
	    if ( m_pcParent->FilterKeyStroke( &cStr ) ) {
		if ( (m_bMultiLine || m_bNumeric == false) || isdigit( cStr[0] )  ||
		     (cStr[0] == '.' && int(m_cBuffer[0].str().find( '.' )) == -1 ) ||
		     (toupper(cStr[0]) == 'E' && int(m_cBuffer[0].str().find( 'e' )) == -1 && int(m_cBuffer[0].str().find( 'E' )) == -1 ) ||
		     ((m_cCsrPos.x == 0 || toupper(m_cBuffer[0][m_cCsrPos.x-1]) == 'E') && (cStr[0] == '+' || cStr[0] == '-')) )
		{
		    if ( m_bRegionActive ) {
			Delete();
		    }
