...
struct RowContentPred { // : public binary_function<ListViewRow,ListViewRow, bool> {
    RowContentPred( int nColumn ) { m_nColumn = nColumn; }
    bool operator()(const ListViewRow* x, const ListViewRow* y) const { return( x->IsLessThan( y, m_nColumn ) ); }
    int m_nColumn;
};

namespace os {
    struct RowPosPred { // : public binary_function<ListViewRow,ListViewRow, bool> {
	bool operator()(const ListViewRow* x, const ListViewRow* y) const { return( x->m_vYPos < y->m_vYPos ); }
    };
}
...
public:
    ...
    virtual float GetWidth( View* pcView, int nColumn ) { return( 0.0f ); }
    virtual float GetHeight( View* pcView ) { return( 0.0f ); }
    virtual void  Paint( const Rect& cFrame, View* pcView, uint nColumn,
			 bool bSelected, bool bHighlighted, bool bHasFocus ) {};
    virtual bool  IsLessThan( const ListViewRow* pcOther, uint nColumn ) const { return( false ); }
...
void ListViewStringRow::Paint( const Rect& cFrame, View* pcView, uint nColumn,
			       bool bSelected, bool bHighlighted, bool bHasFocus )
{
    font_height sHeight;
    pcView->GetFontHeight( &sHeight );
...
ListViewContainer::ListViewContainer( ListView* pcListView, const Rect& cFrame, uint32 nModeFlags ) :
    View( cFrame, "main_view", CF_FOLLOW_NONE, WID_WILL_DRAW | WID_DRAW_ON_CHILDREN )
{
...
void ListViewContainer::MouseDown( const Point& cPosition, uint32 nButton )
{
	...
	ExpandSelect( nHitRow, (nQualifiers & QUAL_CTRL),
		      ((nQualifiers & (QUAL_CTRL | QUAL_SHIFT)) == 0 || (m_nModeFlags & ListView::F_MULTI_SELECT) == 0) );
...
bool ListViewContainer::HandleKey( char nChar, uint32 nQualifiers )
{
	...
	case VK_UP_ARROW:
	{
		...
		ExpandSelect( m_nEndSel - ((nChar == VK_UP_ARROW) ? 1 : nPageHeight),
			      false, (nQualifiers & QUAL_SHIFT) == 0 || (m_nModeFlags & ListView::F_MULTI_SELECT) == 0 );
	    }
	...
	case VK_DOWN_ARROW:
	{
		...
		ExpandSelect( m_nEndSel + ((nChar == VK_DOWN_ARROW) ? 1 : nPageHeight),
			      false, (nQualifiers & QUAL_SHIFT) == 0 || (m_nModeFlags & ListView::F_MULTI_SELECT) == 0 );
	    }
	    if ( (nQualifiers & QUAL_SHIFT) == 0 ) {
		m_nBeginSel = m_nEndSel;
...
void ListViewHeader::DrawButton( const char* pzTitle, const Rect& cFrame, Font* pcFont, font_height* psFontHeight )
{
    ...
    DrawString( pzTitle, nStrLen,
		cFrame.LeftTop() + Point( 5, (cFrame.Height()+1.0f) / 2 - vFontHeight / 2 + psFontHeight->ascender ) );
}
...
ListView::ListView( const Rect& cFrame, const char* pzTitle, uint32 nModeFlags, uint32 nResizeMask, uint32 nFlags ) :
