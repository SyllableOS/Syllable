...
Window::Window( const Rect& cFrame, const std::string& cName, const std::string& cTitle, uint32 nFlags, uint32 nDesktopMask )
	: Looper( cName, DISPLAY_PRIORITY ) //, m_cTitle( cTitle )
{
...
//    Init();
	...
	int hTopView;
	m->m_hLayerPort = Application::GetInstance()->CreateWindow( m->m_pcTopView, cFrame, cTitle, nFlags,
								 nDesktopMask, GetMsgPort(), &hTopView );
...
void Window::Flush()
{
    ...
    if ( send_msg( m->m_hLayerPort, WR_RENDER, m->m_psRenderPkt,
...
View* Window::FindView( const Point& cPos ) const
{
    ...
    for ( View* pcTmpWid = m->m_pcTopView->GetChildAt( cViewPos ) ;
	  pcTmpWid != NULL ;
	  pcTmpWid = pcTmpWid->GetChildAt( cViewPos ) )
    {
...
void Window::_CallMouseMoved( View* pcView, uint32 nButtons, int nWndTransit, Message* pcData )
{
    ...
    if ( (nWndTransit == MOUSE_ENTERED || nWndTransit == MOUSE_INSIDE) &&
	 pcView->GetBounds().DoIntersect( cPos ) ) {
...
TopView::TopView( const Rect& cFrame, Window* pcWindow ) :
    View( cFrame, "window_top_view", CF_FOLLOW_ALL, WID_CLEAR_BACKGROUND )
{
    m_pcWindow = pcWindow;
}
