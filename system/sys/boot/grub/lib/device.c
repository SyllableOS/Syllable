...
get_drive_geometry (struct geometry *geom, char **map, int drive)
{
  ...
  {
    device_geometry  dev_geom;
    if ( ioctl( fd, IOCTL_GET_DEVICE_GEOMETRY, &dev_geom ) < 0 ) {
	goto fail;
    }
    geom->cylinders     = dev_geom.cylinder_count;
    geom->heads         = dev_geom.head_count;
    geom->sectors       = dev_geom.sectors_per_track;
    geom->total_sectors = dev_geom.sector_count;
  }
#else
...
#elif defined(__ATHEOS__)
  sprintf( name, "/dev/disk/bios/hd%c/raw", 'a' + unit );
#else
