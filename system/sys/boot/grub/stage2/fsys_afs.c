...
static afs_off_t afs_run_to_num( const AfsSuperBlock_s* psSuperBlock, const BlockRun_s* psRun )
{
    return( (afs_off_t)psRun->group * psSuperBlock->as_nBlockPerGroup + psRun->start );
}
...
static int get_stream_blocks( AfsSuperBlock_s* psSuperBlock, DataStream_s* psStream,
			      int nPos, int nRequestLen, afs_off_t* pnStart, int* pnActualLength )
{
	...
	if ( nPos < psStream->ds_nMaxIndirectRange ) {
	    afs_off_t   nBlk        = afs_run_to_num( psSuperBlock, &psStream->ds_sIndirect );
	    ...
	    const afs_off_t nIndirectBlk = afs_run_to_num( psSuperBlock, &psStream->ds_sDoubleIndirect ) + nIDBlk;
...
static int do_read( void* a_pBuffer, int nPos, int nSize )
{
    ...
    nError = get_stream_blocks( g_psSuperBlock, &g_psCurInode->ai_sData, nFirstBlock,
...
again:
    ...
    read_blocks( g_psCurInode, nCurInode*g_psSuperBlock->as_nBlockSize, sizeof(AfsInode_s) );
    ...
    for (pzName = pzPath; (ch = *pzName) && !isspace(ch) && ch != '/'; pzName++);
