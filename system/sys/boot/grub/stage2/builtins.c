...
/* The first sector of stage1.5. Set by embed_func() */
static int first_stage1_5_sect;
...
embed_func (char *arg, int flags)
{
  ...
  if (current_partition == 0xFFFFFF)
	  ...
	  || ! fsys_table[fsys_type].embed_func (&start_sector, size))
	{
      ...
      first_stage1_5_sect = start_sector;
		...
		if (installed_partition == 0xFFFFFF)
		...
		else if (fsys_table[fsys_type].embed_func != 0)
		  {
		    ...
		    if (! errnum)
		      {
			/* Construct the blocklist representation.  */
			grub_sprintf (stage2, "%s%d+%d", device, first_stage1_5_sect, blocksize);
