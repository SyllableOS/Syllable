...
thread_id sys_spawn_thread( const char* const pzName, void* const pfEntry,
			    const int nPri, int nStackSize, void* const pData )
{
    ...
    psNewThread->tr_nStackArea = create_area( "stack", &psNewThread->tr_pUserStack, psNewThread->tr_nUStackSize, psNewThread->tr_nUStackSize,
					      AREA_ANY_ADDRESS | AREA_FULL_ACCESS, AREA_NO_LOCK );
...
void db_list_threads( int argc, char** argv )
{
	...
	dbprintf( DBP_DEBUGGER, "%05d - %05d (%04x::%p) %-12s %04d %s::%s\n", hThread, psThread->tr_psProcess->tc_hProcID,
		  psThread->tr_nLastCS, psThread->tr_pLastEIP, pzState,
		  psThread->tr_nPriority, psThread->tr_zName, psThread->tr_psProcess->tc_zName );
    }
