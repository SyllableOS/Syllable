...
//    printf( "Insert: %Ld -> %Ld (%d) (%d)\n", psLock->fl_nStart, psLock->fl_nEnd, psLock->fl_hOwner, bInsert );
	...
	*ppsInsertPoint = psLock;
	...
	if ( psLeft != NULL && psLeft->fl_nEnd + 1 == psLock->fl_nStart &&
	     psLeft->fl_hOwner == psLock->fl_hOwner && psLeft->fl_nMode == psLock->fl_nMode )
	{
	...
	if (  psRight != NULL && psLock->fl_nEnd + 1 == psRight->fl_nStart &&
	      psLock->fl_hOwner == psRight->fl_hOwner && psLock->fl_nMode == psRight->fl_nMode )
	{
