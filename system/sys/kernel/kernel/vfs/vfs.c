...
static int lookup_parent_inode( Inode_s* psParent, const char* pzPath,
				const char** ppzName, int *pnNameLen, Inode_s** ppsResInode );
...
static int cfs_readlink( Inode_s* psInode, char* pzBuffer, int nBufLen )
{
	...
	kassertw( is_semaphore_locked( g_hAreaTableSema ) == false );
	nError =  psInode->i_psOperations->readlink( psInode->i_psVolume->v_pFSData,
						     psInode->i_pFSData, pzBuffer, nBufLen );
    }
    UNLOCK_INODE_RO( psInode );
    return( nError );
}
...
static int cfs_locate_inode( Inode_s* psParent, const char* pzName, int nNameLen,
			     Inode_s** ppsRes, bool bFollowMount )
{
    IoContext_s*  psCtx = get_current_iocxt( false );
    ino_t	  nInodeNum;
    int		  nError;
  
    if ( 2 == nNameLen && ('.' == pzName[0] && '.' == pzName[1]) &&
	 psParent->i_nInode == psParent->i_psVolume->v_nRootInode )
    {
    ...
    kassertw( is_semaphore_locked( g_hAreaTableSema ) == false );
    nError = psParent->i_psOperations->locate_inode( psParent->i_psVolume->v_pFSData,
						     psParent->i_pFSData,
						     pzName, nNameLen, &nInodeNum );
    if ( nError >= 0 ) {
...
static int cfs_create( Inode_s* psParent, const char* pzName, int nNameLen,
		       int nFlags, int nPerms, File_s* psFile )
{
    int nError = -EACCES;
	...
	kassertw( is_semaphore_locked( g_hAreaTableSema ) == false );
	nError =  psParent->i_psOperations->create( psParent->i_psVolume->v_pFSData,
						    psParent->i_pFSData, pzName, nNameLen, nFlags,
						    nPerms & ~CURRENT_THREAD->tr_nUMask,
						    &nInodeNum, &psFile->f_pFSCookie );
	if ( nError >= 0 ) {
...
static int cfs_symlink( Inode_s* psParent, const char* pzName, int nNameLen, const char* pzNewPath )
{
	...
	kassertw( is_semaphore_locked( g_hAreaTableSema ) == false );
	nError = psParent->i_psOperations->symlink( psParent->i_psVolume->v_pFSData,
						    psParent->i_pFSData, pzName, nNameLen, pzNewPath );
    } else {
...
static int cfs_ioctl( File_s* psFile, int nCmd, void* pBuffer, bool bFromKernel )
{
	...
	kassertw( is_semaphore_locked( g_hAreaTableSema ) == false );
	nError = psInode->i_psOperations->ioctl( psInode->i_psVolume->v_pFSData,
						 psInode->i_pFSData, psFile->f_pFSCookie, nCmd, pBuffer, bFromKernel );
    }
    UNLOCK_INODE_RO( psInode );
    return( nError );
}
...
static int cfs_add_select_request( Inode_s* psInode, SelectRequest_s* psReq )
{
    ...
    {
	kassertw( is_semaphore_locked( g_hAreaTableSema ) == false );
	nError = psInode->i_psOperations->add_select_req( psInode->i_psVolume->v_pFSData,
							  psInode->i_pFSData, psReq );
    }
    else
    {
...
static int cfs_rem_select_request( Inode_s* psInode, SelectRequest_s* psReq )
{
    ...
    {
	kassertw( is_semaphore_locked( g_hAreaTableSema ) == false );
	nError =  psInode->i_psOperations->rem_select_req( psInode->i_psVolume->v_pFSData,
							   psInode->i_pFSData, psReq );
	UNLOCK_INODE_RO( psInode );
    }
    return( nError );
...
int open_inode( bool bKernel, Inode_s* psInode, int nType, int nMode )
{
	...
	kassertw( is_semaphore_locked( g_hAreaTableSema ) == false );
	nError = psInode->i_psOperations->open( psInode->i_psVolume->v_pFSData,
						psInode->i_pFSData, nMode, &psFile->f_pFSCookie );
	if ( nError < 0 ) {
	    goto error1;
	}
    }
...
static int open_file( Inode_s** ppsParent, const char* pzName, int nNameLen,
		      int nFlags, File_s* psFile )
{
    int	   nError = 0;
	...
	if ( (*ppsParent)->i_psOperations->open != NULL ) {
	    kassertw( is_semaphore_locked( g_hAreaTableSema ) == false );
	    nError = (*ppsParent)->i_psOperations->open( (*ppsParent)->i_psVolume->v_pFSData,
							 (*ppsParent)->i_pFSData, nFlags, &psFile->f_pFSCookie );
	}
	if ( nError >= 0 ) {
	...
	if ( psInode->i_psOperations->open != NULL ) {
	    kassertw( is_semaphore_locked( g_hAreaTableSema ) == false );
	    nError = psInode->i_psOperations->open( psInode->i_psVolume->v_pFSData,
						    psInode->i_pFSData, nFlags, &psFile->f_pFSCookie );
	}
	if ( nError >= 0 ) {
	    add_file_to_inode( psInode, psFile );
	    psFile->f_psInode = psInode;
...
int sys_rename( const char *a_pzOldName, const char* a_pzNewName )
{
    ...
    kassertw( is_semaphore_locked( g_hAreaTableSema ) == false );
    nError = psOldDir->i_psOperations->rename( psOldDir->i_psVolume->v_pFSData,
					       psOldDir->i_pFSData, pzOldName, nOldLen,
					       psNewDir->i_pFSData, pzNewName, nNewLen, bMustBeDir );
error6:
...
ssize_t	read_pos_p( File_s* psFile, off_t nPos, void* pBuffer, size_t nLength )
{
    ...
    kassertw( is_semaphore_locked( g_hAreaTableSema ) == false );
    nError = psInode->i_psOperations->read( psInode->i_psVolume->v_pFSData, psInode->i_pFSData,
					    psFile->f_pFSCookie, nPos, pBuffer, nLength );
    if ( nError < 0 ) {
	goto error;
    }
error:
...
ssize_t	readv_pos_p( File_s* psFile, off_t nPos, const struct iovec* psVector, int nCount )
{
	...
	kassertw( is_semaphore_locked( g_hAreaTableSema ) == false );
	nError = psInode->i_psOperations->readv( psInode->i_psVolume->v_pFSData, psInode->i_pFSData,
						 psFile->f_pFSCookie, nPos, psVector, nCount );
    }
error:
    UNLOCK_INODE_RO( psInode );
    return( nError );
...
ssize_t	write_pos_p( File_s* psFile, off_t nPos, const void* pBuffer, size_t nLength )
{
    ...
    kassertw( is_semaphore_locked( g_hAreaTableSema ) == false );
    nError = psInode->i_psOperations->write( psInode->i_psVolume->v_pFSData, psInode->i_pFSData,
					     psFile->f_pFSCookie, nPos, pBuffer, nLength );
error:
    UNLOCK_INODE_RO( psInode );
    return( nError );
...
ssize_t	writev_pos_p( File_s* psFile, off_t nPos, const struct iovec* psVector, int nCount )
{
	...
	nError = psInode->i_psOperations->writev( psInode->i_psVolume->v_pFSData, psInode->i_pFSData,
						  psFile->f_pFSCookie, nPos, psVector, nCount );
    }
error:
    UNLOCK_INODE_RO( psInode );
...
static int do_getdents( bool bKernel, int nFile, struct kernel_dirent *psDirEnt, int nBufSize )
{
    ...
    kassertw( is_semaphore_locked( g_hAreaTableSema ) == false );
    nError = psInode->i_psOperations->readdir( psInode->i_psVolume->v_pFSData, psInode->i_pFSData,
					       psFile->f_pFSCookie, psFile->f_nPos,
					       &sEntry, sizeof( struct kernel_dirent ) );
...
int do_rewinddir( bool bKernel, int nFile )
{
    ...
    kassertw( is_semaphore_locked( g_hAreaTableSema ) == false );
    nError = psInode->i_psOperations->rewinddir( psInode->i_psVolume->v_pFSData, psInode->i_pFSData,
						 psFile->f_pFSCookie );
    psFile->f_nPos = 0;
    UNLOCK_INODE_RO( psInode );
error:
...
int sys_chmod( const char* a_pzPath, const mode_t nMode )
{
    ...
    if ( psProc->pr_nFSUID != sStat.st_uid &&
	 is_in_group( sStat.st_gid ) == false &&
	 is_root( true ) == false )
    {
	nError = -EPERM;
	goto error1;
...
int sys_mknod( const char* a_pzPath, int nMode, dev_t nDev )
{
    ...
    kassertw( is_semaphore_locked( g_hAreaTableSema ) == false );
    nError = psParent->i_psOperations->mknod( psParent->i_psVolume->v_pFSData, psParent->i_pFSData,
					      pzName, nLen, nMode & ~CURRENT_THREAD->tr_nUMask, nDev );
...
static int do_mkdir( Inode_s* psRoot, const char* pzPath, mode_t nMode )
{
    ...
    kassertw( is_semaphore_locked( g_hAreaTableSema ) == false );
    nError = psParent->i_psOperations->mkdir( psParent->i_psVolume->v_pFSData, psParent->i_pFSData,
					      pzName, nLen, nMode & ~CURRENT_THREAD->tr_nUMask );
...
static int set_file_flags( File_s* psFile, int nFlags )
{
	...
	kassertw( is_semaphore_locked( g_hAreaTableSema ) == false );
	nError = psInode->i_psOperations->setflags( psInode->i_psVolume->v_pFSData, psInode->i_pFSData,
						    psFile->f_pFSCookie, psFile->f_nMode );
    }
    UNLOCK_INODE_RO( psInode );
    return( nError );
}
...
int sys_fcntl( int nFile, int nCmd, int nArg )
{
	...
	case F_SETFL:
	    ...
	    nError = set_file_flags( psFile,
				     (psFile->f_nMode & ~(O_APPEND | O_NONBLOCK)) | (nArg & (O_APPEND | O_NONBLOCK)) );
	    break;
...
int sys_select( int nMaxCnt, fd_set* psReadSet, fd_set* psWriteSet,
		fd_set* psExceptSet, struct kernel_timeval* psTimeOut )
{
    int		   nRealCnt = 0;
    int		   nReadyCount = 0;
...
int get_file_blocks_p( File_s* psFile, off_t nPos, int nBlockCount, off_t* pnStart, int* pnActualCount )
{
    ...
    kassertw( is_semaphore_locked( g_hAreaTableSema ) == false );
    nError = psInode->i_psOperations->get_file_blocks( psInode->i_psVolume->v_pFSData, psInode->i_pFSData,
						       nPos, nBlockCount, pnStart, pnActualCount );
    UNLOCK_INODE_RO( psInode );
    return( nError );
}
