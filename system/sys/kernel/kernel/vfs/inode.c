...
void dump_hash_table( HashTable_s* psTable )
{
	...
	if ( psTable->ht_apsTable[i] != NULL ) {
		...
		dbprintf( DBP_DEBUGGER, "%03d::%08Ld cnt=%d vol=%p mnt=%p vno=%p busy=%d\n",
			  ...
			  psInode->i_nCount, psInode->i_psVolume,
			  psInode->i_psMount, psInode->i_pFSData, psInode->i_bBusy );
	    }
	}
    }
}
...
Inode_s* do_get_inode( kdev_t nDevNum, ino_t nInoNum, bool bCrossMount, bool bOkToFlush )
{
	...
	else
	{
		...
		if ( NULL != psInode ) {
		    ...
		    UNLOCK( g_hInodeHashSem );
		    kassertw( is_semaphore_locked( g_hAreaTableSema ) == false );
		    nError = psInode->i_psOperations->read_inode( psInode->i_psVolume->v_pFSData,
								  nInoNum, &psInode->i_pFSData );
...
int sys_initialize_fs( const char* pzDevPath, const char* pzFsType,
		       const char* pzVolName, void* pArgs, int nArgLen )
{
    FileSysDesc_s* psDesc;
    int		 nError;
...
static int do_mount( const char* pzDevName, const char* pzDirName,
		     const char* pzFSName, int nFlags, int nFSID, void* pData )
{
    Volume_s* 	 psVol;
    FileSysDesc_s* psDesc;
    Inode_s*	 psDir;
...
static bool check_fs_open_files( Volume_s* psVolume )
{
    ...
    for ( i = 0 ; i < g_sHashTable.ht_nSize && psInode == NULL ; ++i ) {
	for ( psInode = g_sHashTable.ht_apsTable[i] ; psInode != NULL ; psInode = psInode->i_psHashNext ) {
	    if ( psInode->i_psVolume == psVolume && psInode->i_nCount > 0 &&
		 (psInode->i_nInode != psVolume->v_nRootInode || psInode->i_nCount > 1) ) {
		return( true );
	    }
	}
    }
