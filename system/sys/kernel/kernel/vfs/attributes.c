...
int sys_fs_open_attrdir( const char* pzName )
{
    ...
    nError = psInode->i_psOperations->open_attrdir( psInode->i_psVolume->v_pFSData, psInode->i_pFSData,
						    &psFile->f_pAttribCookie );
    if ( nError < 0 ) {
	goto error3;
    }
    UNLOCK_INODE_RO( psInode );
...
int sys_close_attrdir( int nFile )
{
    ...
    kassertw( is_semaphore_locked( g_hAreaTableSema ) == false );
    nError = psInode->i_psOperations->close_attrdir( psInode->i_psVolume->v_pFSData, psInode->i_pFSData,
						     psFile->f_pAttribCookie );
    UNLOCK_INODE_RO( psInode );
...
int sys_read_attrdir( int nFile, struct kernel_dirent* psBuffer, int nCount )
{
    ...
    kassertw( is_semaphore_locked( g_hAreaTableSema ) == false );
    nError = psInode->i_psOperations->read_attrdir( psInode->i_psVolume->v_pFSData, psInode->i_pFSData,
						    psFile->f_pFSCookie, psBuffer, nCount * sizeof( *psBuffer ) );
    UNLOCK_INODE_RO( psInode );
    put_fd( psFile );
    return( nError );
}
...
int sys_remove_attr( int nFile, const char* pzName )
{
    ...
    kassertw( is_semaphore_locked( g_hAreaTableSema ) == false );
    nError = psInode->i_psOperations->remove_attr( psInode->i_psVolume->v_pFSData, psInode->i_pFSData,
...
int sys_stat_attr( int nFile, const char* pzName, struct attr_info* psBuffer )
{
    ...
    kassertw( is_semaphore_locked( g_hAreaTableSema ) == false );
    nError = psInode->i_psOperations->stat_attr( psInode->i_psVolume->v_pFSData, psInode->i_pFSData,
						 pzName, strlen( pzName ), psBuffer );
    UNLOCK_INODE_RO( psInode );
    put_fd( psFile );
    return( nError );
}
...
static int do_write_attr( int nFile, const char* pzName, int nFlags, int nType,
			   const void* pBuffer, off_t nPos, size_t nLen )
{
    ...
    kassertw( is_semaphore_locked( g_hAreaTableSema ) == false );
    nError = psInode->i_psOperations->write_attr( psInode->i_psVolume->v_pFSData, psInode->i_pFSData,
						  pzName, strlen( pzName ), nFlags, nType, pBuffer, nPos, nLen );
    UNLOCK_INODE_RO( psInode );
    put_fd( psFile );
    return( nError );
}
...
{
    return( do_write_attr( psParams->wa_nFile, psParams->wa_pzName, psParams->wa_nFlags, psParams->wa_nType,
			   psParams->wa_pBuffer, psParams->wa_nPos, psParams->wa_nLen ) );
}
...
static int do_read_attr( int nFile, const char* pzName, int nType,
			 void* pBuffer, off_t nPos, size_t nLen )
{
    ...
    kassertw( is_semaphore_locked( g_hAreaTableSema ) == false );
    nError = psInode->i_psOperations->read_attr( psInode->i_psVolume->v_pFSData, psInode->i_pFSData,
						 pzName, strlen( pzName ), nType, pBuffer, nPos, nLen );
    UNLOCK_INODE_RO( psInode );
    put_fd( psFile );
    return( nError );
}
...
{
    return( do_read_attr( psParams->wa_nFile, psParams->wa_pzName, psParams->wa_nType,
			  psParams->wa_pBuffer, psParams->wa_nPos, psParams->wa_nLen ) );
}
