...
int sys_setpgid( pid_t a_hDest, pid_t a_hGroup )
{
	...
	FOR_EACH_THREAD( hTmp, psTmp ) {
	    if ( psTmp->tr_psProcess->pr_hPGroupID == hGroup &&
		 psTmp->tr_psProcess->pr_nSession == psThisProc->pr_nSession )
	    {
		bGroupOK = true;
		break;
	    }
...
//  int old_suid = psProc->pr_nSUID;
    ...
    if (!is_root(false)) {
	if ((ruid != (uid_t) -1) && (ruid != psProc->pr_nUID) &&
	    (ruid != psProc->pr_nEUID) && (ruid != psProc->pr_nSUID))
	    return -EPERM;
	if ((euid != (uid_t) -1) && (euid != psProc->pr_nUID) &&
	    (euid != psProc->pr_nEUID) && (euid != psProc->pr_nSUID))
	    return -EPERM;
	if ((suid != (uid_t) -1) && (suid != psProc->pr_nUID) &&
	    (suid != psProc->pr_nEUID) && (suid != psProc->pr_nSUID))
	    return -EPERM;
    }
    if (ruid != (uid_t) -1) {
...
int sys_getresuid(uid_t *ruid, uid_t *euid, uid_t *suid)
{
    ...
    if (!(retval = memcpy_to_user( ruid, &psProc->pr_nUID, sizeof(psProc->pr_nUID) )) &&
	!(retval = memcpy_to_user( euid, &psProc->pr_nEUID, sizeof(psProc->pr_nEUID) )))
...
int sys_setresgid(gid_t rgid, gid_t egid, gid_t sgid)
{
    ...
    if (!is_root(false)) {
	if ((rgid != (gid_t) -1) && (rgid != psProc->pr_nGID) &&
	    (rgid != psProc->pr_nEGID) && (rgid != psProc->pr_nSGID))
	    return -EPERM;
	if ((egid != (gid_t) -1) && (egid != psProc->pr_nGID) &&
	    (egid != psProc->pr_nEGID) && (egid != psProc->pr_nSGID))
	    return -EPERM;
	if ((sgid != (gid_t) -1) && (sgid != psProc->pr_nGID) &&
	    (sgid != psProc->pr_nEGID) && (sgid != psProc->pr_nSGID))
	    return -EPERM;
    }
    if (rgid != (gid_t) -1)
	psProc->pr_nGID = rgid;
...
int sys_getresgid(gid_t *rgid, gid_t *egid, gid_t *sgid)
{
    ...
    if (!(retval = memcpy_to_user( rgid, &psProc->pr_nGID, sizeof(psProc->pr_nGID) )) &&
	!(retval = memcpy_to_user( egid, &psProc->pr_nEGID, sizeof(psProc->pr_nEGID) )))
...
int sys_setfsuid(uid_t uid)
{
    ...
    if (uid == psProc->pr_nUID || uid == psProc->pr_nEUID ||
	uid == psProc->pr_nSUID || uid == psProc->pr_nFSUID || 
	is_root(false) )
...
int sys_setfsgid(gid_t gid)
{
    ...
    if (gid == psProc->pr_nGID || gid == psProc->pr_nEGID ||
	gid == psProc->pr_nSGID || gid == psProc->pr_nFSGID || 
	is_root(false))
