...
static int init_hash_table( HashTable_s* psTable )
{
    ...
    psTable->ht_apsTable = kmalloc( psTable->ht_nSize * sizeof(CacheBlock_s*),
...
static int flush_block_list( CacheBlock_s** pasBlocks, int nCount )
{
	...
	if ( i > 0 && pasBlocks[i-1]->cb_nDevice == pasBlocks[i]->cb_nDevice &&
	     pasBlocks[i-1]->cb_nBlockNum >= pasBlocks[i]->cb_nBlockNum )
	{
	...
	if ( NULL != psPrevBuffer &&
	     (psPrevBuffer->cb_nDevice != psBuffer->cb_nDevice ||
	      psPrevBuffer->cb_nBlockNum != (psBuffer->cb_nBlockNum - 1)) )
	{
	    ...
	    nError = writev_pos( psFirstBuffer->cb_nDevice, psFirstBuffer->cb_nBlockNum * CB_SIZE( psFirstBuffer ),
				 pasIoVecs, nIoVecCnt );
	...
	if ( psPrevBuffer->cb_nBlockNum - psFirstBuffer->cb_nBlockNum != nIoVecCnt - 1 )
	{
	    ...
	    for ( j = 0 ; j < nCount ; j++ ) {
		printk( "%d -> FLG=%02ld, RC=%02d, Num=%Ld\n",
			j, pasBlocks[j]->cb_nFlags,
			pasBlocks[j]->cb_nRefCount, pasBlocks[j]->cb_nBlockNum );
	    }
...
static void dump_list( CacheEntryList_s* psList )
{
    ...
    {
	printk( "%d -> FLG=%02ld, RC=%02d, Num=%Ld\n", i++, psEntry->cb_nFlags,
		psEntry->cb_nRefCount, psEntry->cb_nBlockNum );
    }
}
...
int set_blocks_info( int nDev, off_t* panBlocks, int nCount, bool bDoClone,
