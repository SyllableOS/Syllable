...
void tcp_dump_packet( PacketBuf_s* psPkt )
{
    ...
    if ( psTCPHdr->tcp_code & TCPF_URG )	strcat( zFlags, "U" );
    if ( psTCPHdr->tcp_code & TCPF_ACK )	strcat( zFlags, "A" );
    if ( psTCPHdr->tcp_code & TCPF_PSH )	strcat( zFlags, "P" );
    if ( psTCPHdr->tcp_code & TCPF_RST )	strcat( zFlags, "R" );
...
//	printk( "Shutdown of receiver\n" );
    ...
    if (psTCPCtrl->tcb_nState == TCPS_FINWAIT1 ||
	psTCPCtrl->tcb_nState == TCPS_FINWAIT2 ||
	psTCPCtrl->tcb_nState == TCPS_CLOSING  ||
	psTCPCtrl->tcb_nState == TCPS_CLOSED   ||
	psTCPCtrl->tcb_nState == TCPS_LISTEN   ||
	psTCPCtrl->tcb_nState == TCPS_LASTACK  ||
	psTCPCtrl->tcb_nState == TCPS_TIMEWAIT )
    {
	goto done;
    }
...
static int  tcp_getpeername( Socket_s* psSocket, struct sockaddr* psName, int* pnNameLen )
{
    ...
    if ( psTCPCtrl->tcb_nState == TCPS_CLOSED || psTCPCtrl->tcb_nState == TCPS_LISTEN ) { // Is this enough???
	nError = -ENOTCONN;
	goto error;
    }
