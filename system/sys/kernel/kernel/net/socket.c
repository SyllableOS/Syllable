...
int create_socket( bool bKernel, int nFamily, int nType, int nProtocol, bool bInitProtocol, Socket_s** ppsRes )
{
	...
	switch( nType ) {
		...
		break;
	    default:
		printk( "create_socket() unknown type %d\n", nType );
		return( -EINVAL );
	}
    }
    ...
    if ( psSocket == NULL ) {
	printk( "Error: sys_socket() ran out of memory\n" );
	return( -ENOMEM );
    }
	...
	switch( nProtocol )
	{
		...
		break;
	    default:
		printk( "create_socket() unknown protocol %d\n", nProtocol );
		nError = -EINVAL;
		break;
	}
    }
    ...
    if ( psFile == NULL ) {
	printk( "Error: sys_socket() no memory for file struct\n" );
	kfree( psSocket );
	return( -ENOMEM );
    }
	...
	printk( "sys_socket() failed to get socket inode\n" );
	free_fd( psFile );
	kfree( psSocket );
	return( -ENOMEM );
    }
...
{
//  printk( "Error: getsockopt() not implemented\n" );
    return( -ENOSYS );
}
...
int  do_connect( bool bKernel, int nFile, const struct sockaddr* psAddr, int nAddrSize )
{
	...
	printk( "sys_connect() unknown family %04x\n", psInAddr->sin_family );
	nError = -EAFNOSUPPORT;
	goto error1;
    }
...
static int do_bind( bool bKernel, int nFile, const struct sockaddr* psAddr, int nAddrSize )
{
    ...
    if ( psInAddr->sin_family != AF_INET ) {
	printk( "sys_bind() unknown family %04x\n", psInAddr->sin_family );
	nError = -EAFNOSUPPORT;
	goto error1;
    }
...
static int sk_add_select( void* pVolume, void* pNode, SelectRequest_s* psReq )
{
	...
	printk( "Error: Protocol %d dont support add_select\n", psSocket->sk_nProto );
	nError = -EINVAL;
    }
...
static int sk_rem_select( void* pVolume, void* pNode, SelectRequest_s* psReq )
{
	...
	printk( "Error: Protocol %d dont support rem_select\n", psSocket->sk_nProto );
	nError = -EINVAL;
    }
...
static int sk_setflags( void* pVolume, void* pNode, void* pCookie, uint32 nFlags )
{
	...
	printk( "Error: Protocol %d dont support rem_setflags\n", psSocket->sk_nProto );
	nError = -EINVAL;
    }
...
void init_sockets()
{
    ...
    if ( NULL == psKVolume ) {
	printk( "Error: Failed to create socket fs-volume\n" );
	return;
    }
