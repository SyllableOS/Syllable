...
RawPort_s *raw_find_port(uint8 a_nIPProtocol)
{
    ...
    LOCK(g_hRawPortLock);
    for (psRawPort = g_psFirstRawPort; psRawPort != NULL;
	 psRawPort = psRawPort->rp_psNext) {
...
static void raw_delete_port(RawPort_s * a_psRawPort)
{
    ...
    for (ppsTempPort = &g_psFirstRawPort; *ppsTempPort != NULL;
	 ppsTempPort = &(*ppsTempPort)->rp_psNext) {
...
static void raw_delete_endpoint(RawEndPoint_s * a_psRawEndPoint)
{
	...
	for (ppsRawEndPoint = &psRawPort->rp_psFirstEndPoint;
	     *ppsRawEndPoint != NULL;
	     ppsRawEndPoint = &(*ppsRawEndPoint)->re_psNext) {
...
static void
raw_add_endpoint(RawPort_s * a_psRawPort, RawEndPoint_s * a_psRawEndPoint)
{
...
static int
raw_bind(Socket_s * a_psSocket, const struct sockaddr *a_psAddress,
	 int a_nAddressSize)
{
...
static int
raw_getsockname(Socket_s * a_psSocket, struct sockaddr *a_psName,
		int *a_pnNameLength)
{
...
static int
raw_connect(Socket_s * a_psSocket, const struct sockaddr *a_psAddress,
	    int a_nAddressSize)
{
    RawEndPoint_s *psRawEndPoint = a_psSocket->sk_psRawEndP;
    RawPort_s *psRawPort = NULL;
...
static int
raw_setsockopt(bool a_bFromKernel, Socket_s * a_psSocket, int a_nProtocol,
	       int a_nOptionName, const void *a_pOptionValue,
	       int a_nOptionLength)
{
    RawEndPoint_s *psRawEndPoint = a_psSocket->sk_psRawEndP;
...
static ssize_t
raw_recvmsg(Socket_s * a_psSocket, struct msghdr *a_psMsg, int a_nFlags)
{
	...
	psPacket =
	    remove_head_packet(&psRawEndPoint->re_sPackets,
			       INFINITE_TIMEOUT);
    }
...
static ssize_t
raw_sendmsg(Socket_s * a_psSocket, const struct msghdr *a_psMsg,
	    int a_nFlags)
{
	...
	(struct sockaddr_in *) a_psMsg->msg_name;
    ...
    if ((psAddress == NULL
	 || IP_SAMEADDR(psAddress->sin_addr, anNullAddress))
	&& IP_SAMEADDR(psRawEndPoint->re_anRemoteAddr, anNullAddress)) {
    ...
    if ((psAddress != NULL
	 && IP_SAMEADDR(psAddress->sin_addr, anNullAddress) == false)
	&& IP_SAMEADDR(psRawEndPoint->re_anRemoteAddr,
		       anNullAddress) == false) {
	...
	psPacket->pb_uNetworkHdr.pRaw + sizeof(IpHeader_s);
	...
	memcpy(pDestinationBuffer, a_psMsg->msg_iov[i].iov_base,
	       a_psMsg->msg_iov[i].iov_len);
...
// <FIXME>
    ...
    psIPHeader->iph_nCheckSum =
	ip_fast_csum((uint8 *) psIPHeader, psIPHeader->iph_nHdrSize);
...
static void
raw_enqueue_packet(RawPort_s * a_psRawPort, PacketBuf_s * a_psPacket)
{
    RawEndPoint_s *psRawEndPoint;
    ...
    for (psRawEndPoint = a_psRawPort->rp_psFirstEndPoint;
	 psRawEndPoint != NULL; psRawEndPoint = psRawEndPoint->re_psNext) {
...
int raw_open(Socket_s * a_psSocket)
{
    ...
    a_psSocket->sk_psRawEndP =
	kmalloc(sizeof(RawEndPoint_s), MEMF_KERNEL | MEMF_CLEAR);
