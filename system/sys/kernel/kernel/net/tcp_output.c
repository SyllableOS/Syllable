...
enum { TSF_NEWDATA, TSF_REXMT, TSF_KEEPALIVE };
...
//      datalen = 65536;
	...
	*pnOffset -= psTCPCtrl->tcb_nSndBufSize;
    ...
    } else if (enq(psTCPCtrl->tcb_sudq, puqe,
		   SUDK(psTCPCtrl, puqe->uq_seq)) < 0)
	uqfree(puqe);	/* shouldn't happen */
...
static int tcp_send( TCPCtrl_s* psTCPCtrl, int nMode )
{
	...
	SEQCMP( ntohl( psTCPHdr->tcp_seq ) + nDataLen, psTCPCtrl->tcb_slast ) == 0 ) {
    ...
    if ( psTCPCtrl->tcb_sndbuf != NULL ) { // Should not be needed
	char*	 pch;
	...
	if ( nEvent == DELETE ) {
		...
		case TCPO_IDLE:
			...
			printk( "Send first keepalive to %d.%d.%d.%d:%d\n",
			       psTCPCtrl->tcb_rip[0],psTCPCtrl->tcb_rip[1],psTCPCtrl->tcb_rip[2],psTCPCtrl->tcb_rip[3],
			       psTCPCtrl->tcb_rport );
