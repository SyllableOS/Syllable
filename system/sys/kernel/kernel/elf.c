...
//	printk( "Unknown dyamic entry %d, %d\n", psDyn->d_nTag, psDyn->d_un.d_nValue );
	...
	if ( nDynRelSize > 0 ) {
	    nError = read_image_data( psImage, psImage->im_pasRelocs,
				      nDynRelAddress - psImage->im_nVirtualAddress + psImage->im_nOffset, nDynRelSize );
	...
	if ( nDynPltRelSize > 0 ) {
	    nError = read_image_data( psImage, ((char*)psImage->im_pasRelocs) + nDynRelSize, 
				      nDynPltRelAddress - psImage->im_nVirtualAddress + psImage->im_nOffset,
				      nDynPltRelSize );
...
static int parse_section_headers( ElfImage_s* psImage )
{
	...
	nError =  read_image_data( psImage, psImage->im_pzStrings,
				   psStrSection->sh_nOffset, psStrSection->sh_nSize );
...
again:
	...
	if ( NULL != psImage ) {
	    if ( psImage->im_nOpenCount == -1 ) {	// Image is about to being loaded
		UNLOCK( g_hImageList );
		snooze( 1000LL );
		goto again;
	    } else {
    ...
    nError = read_image_data( psImage, psImage->im_pasSections, sElfHdr.e_nSecHdrOff,
			      sElfHdr.e_nSecHdrCount * sizeof( Elf32_SectionHeader_s ) );
...
static status_t	memmap_instance( ElfImageInst_s* psInst, int nMode )
{
    ...
    nError = alloc_area_list( nAllocMode, nLocking, nAllocAddress, nAreaCount,
			      apzAreaNames, anAreaOffsets, anAreaSizes, anAreas );
    if ( nError < 0 ) {
	...
	nError = read_image_data( psImage, sAreaInfo.pAddress,
				  nTextOffset, nFileTextSize );
...
static char* get_sym_address( ElfImageInst_s* psInst, ElfSymbol_s* psSym )
{
    ...
    if ( psInst->ii_nTextArea == -1 ) { // Only true for kernel symbols
...
static int do_reloc_image( ImageContext_s* psCtx, ElfImageInst_s* psInst, int nMode, ElfImageInst_s* psParent,
			   Elf32_Reloc_s* psRelTab, int nRelCount )
{
    ElfImage_s* psImage = psInst->ii_psImage;
    int	        nError;
    int	        i;
	...
	switch( nRelocType )
	{
		...
		*pTarget = (uint32)get_sym_address( psDllInst, psDllSym );
	    ...
	    case R_386_RELATIVE:
		if ( *pTarget != 0 ) { // Not quite sure if this is correct, but it made C++ exceptions work...
...
int load_image_inst( ImageContext_s* psCtx, const char* pzPath, BootModule_s* psModule,
		     int nMode, ElfImageInst_s** ppsInst, const char* pzLibraryPath )
{
    ElfImage_s*     psImage;
	...
	*ppsInst = psInst;
    ...
    psInst->ii_apsSubImages = kmalloc( psImage->im_nSubImageCount * sizeof( ElfImageInst_s* ),
				       MEMF_KERNEL | MEMF_CLEAR | MEMF_OKTOFAILHACK );
    ...
    for ( i = 0 ; i < psImage->im_nSubImageCount ; ++i ) {
	if ( nMode == IM_KERNEL_SPACE ) {
	    nError = load_image_inst( psCtx, psImage->im_apzSubImages[i], NULL,
				      IM_KERNEL_SPACE, &psInst->ii_apsSubImages[i], pzLibraryPath );
	} else {
	    nError = load_image_inst( psCtx, psImage->im_apzSubImages[i], NULL,
				      IM_LIBRARY_SPACE, &psInst->ii_apsSubImages[i], pzLibraryPath );
	}
	if ( nError < 0 ) {
	    int j;
	    for ( j = 0 ; j < i ; ++j ) {
...
ImageContext_s* clone_image_context( ImageContext_s* psOrig, MemContext_s* psNewMemCtx )
{
	...
	for ( i = 0 ; i < MAX_IMAGE_COUNT ; ++i ) {
		...
		psDst->ii_apsSubImages = kmalloc( psDst->ii_psImage->im_nSubImageCount * sizeof( ElfImageInst_s* ),
						  MEMF_KERNEL | MEMF_CLEAR | MEMF_OKTOFAILHACK );
...
static int do_find_module_by_address( ImageContext_s* psCtx, uint32 nAddress )
{
	...
	if ( psInst->ii_nTextArea != -1 && get_area_info( psInst->ii_nTextArea, &sAreaInfo ) >= 0 &&
	     nAddress >= ((uint32)sAreaInfo.pAddress) && nAddress < ((uint32)sAreaInfo.pAddress) + sAreaInfo.nSize ) {
	    break;
	}
	if ( psInst->ii_nDataArea != -1 && get_area_info( psInst->ii_nDataArea, &sAreaInfo ) >= 0 &&
	     nAddress >= ((uint32)sAreaInfo.pAddress) && nAddress < ((uint32)sAreaInfo.pAddress) + sAreaInfo.nSize ) {
	    break;
	}
	psInst = NULL;
...
void db_list_process_images( Process_s* psProc )
{
	...
	if ( psCtx->ic_psInstances[i] != NULL )
	{
	    ...
	    dbprintf( DBP_DEBUGGER, "%03d %02d %02d %08lx %08lx %04d %04d %s\n",
		      i, psInst->ii_nOpenCount, psInst->ii_nAppOpenCount,
		      psInst->ii_nTextAddress, psImage->im_nVirtualAddress, psImage->im_nSymCount,
		      psImage->im_nRelocCount, psImage->im_zName );
	}
    }
...
static void init_boot_modules()
{
    ...
    for ( i = 0 ; i < MAX_IMAGE_COUNT ; ++i ) {
	if ( g_psKernelCtx->ic_psInstances[i] != NULL &&
	     strstr( g_psKernelCtx->ic_psInstances[i]->ii_psImage->im_pzPath, "/drivers/dev/" ) != NULL ) {
