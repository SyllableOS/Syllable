...
struct uhci_qh {
	/* Hardware fields */
	uint32 link;			/* Next queue */
	uint32 element;			/* Queue element pointer */
...
struct uhci {
	...
	SpinLock_s frame_list_lock;
	void* fl_real;
	struct uhci_frame_list *fl;		/* Frame list */
	int fsbr;				/* Full speed bandwidth reclamation */
	int is_suspended;

	SpinLock_s qh_remove_list_lock;
	struct list_head qh_remove_list;

	SpinLock_s urb_remove_list_lock;
	struct list_head urb_remove_list;

	SpinLock_s urb_list_lock;
	struct list_head urb_list;

	SpinLock_s complete_list_lock;
...
struct urb_priv {
	...
	uint32 setup_packet_dma_handle;
	uint32 transfer_buffer_dma_handle;
	...
	bigtime_t inserttime;	/* In jiffies */
	bigtime_t fsbrtime;	/* In jiffies */
