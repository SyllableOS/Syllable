...
periodic_next_shadow (union ehci_shadow *periodic, int tag)
{
	...
	kerndbg( KERN_DEBUG, "BAD shadow %p tag %d\n", periodic->ptr, tag);
	// BUG ();
	return 0;
}
...
static int periodic_unlink (struct ehci_hcd *ehci, unsigned frame, void *ptr)
{
	...
	if (!here.ptr) {
		kerndbg( KERN_DEBUG, "entry %p no longer on frame [%d]\n", ptr, frame);
		return 0;
	}
	...
	while (q->ptr) {
		...
		case Q_TYPE_FSTN:
			...
			if (q->fstn->hw_prev != EHCI_LIST_END) {
				kerndbg( KERN_DEBUG, "not counting FSTN bandwidth yet ...");
			}
			q = &q->fstn->fstn_next;
			break;
		case Q_TYPE_ITD:
...
static void intr_deschedule (
	...
	hcd_to_bus (&ehci->hcd)->nBandWidthAllocated -= 
...
static int qh_schedule (struct ehci_hcd *ehci, struct ehci_qh *qh)
{
	...
	if (status) {
		...
		kerndbg( KERN_DEBUG, "reused previous qh %p schedule\n", qh);

	/* stuff into the periodic schedule */
	qh->qh_state = QH_STATE_LINKED;
	kerndbg( KERN_DEBUG, "scheduled qh %p usecs %d/%d period %d.0 starting %d.%d (gap %d)\n",
		qh, qh->usecs, qh->c_usecs,
		qh->period, frame, uframe, qh->gap_uf);
	do {
		if (ehci->pshadow [frame].ptr != 0) {
...
intr_complete (
	...
	/* nothing to report? */
	if ((qh->hw_token & cpu_to_le32 (QTD_STS_ACTIVE))
			!= 0)
		return 0;
	if (list_empty (&qh->qtd_list)) {
		kerndbg( KERN_DEBUG, "intr qh %p no TDs?\n", qh);
		return 0;
	}
...
static void
itd_free_list (struct ehci_hcd *ehci, USB_packet_s *urb)
{
	struct ehci_itd *first_itd = urb->pHCPrivate;
...
itd_fill (
	...
	uint32	dma		// mapped transfer buffer
) {
	u64		temp;
	u32		buf1;
	...
	length = urb->sISOFrame[index].nLength;
	dma += urb->sISOFrame[index].nOffset;
	temp = dma & ~0x0fff;
	for (i = 0; i < 2; i++) {
	...
	is_input = usb_pipein (urb->nPipe);
	if (is_input) {
		maxp = urb->psDevice->nMaxPacketIn[epnum];
		buf1 = (1 << 11);
	} else {
		maxp = urb->psDevice->nMaxPacketOut[epnum];
		buf1 = 0;
	}
	buf1 |= (maxp & 0x03ff);
	multi = 1;
	...
	if (length < 0 || maxp < length) {
		kerndbg( KERN_WARNING, "BAD iso packet: %d bytes, max %d, urb %p [%d] (of %d)\n",
			length, maxp, urb, index,
			urb->sISOFrame[index].nLength);
		return -ENOSPC;
	}
	itd->usecs = hcd_calc_bus_time (USB_SPEED_HIGH, is_input, 1, length);

	/* "plus" info in low order bits of buffer pointers */
	itd->hw_bufp [0] |= cpu_to_le32 ((epnum << 8) | urb->psDevice->nDeviceNum);
...
itd_urb_transaction (
	...
	for (frame_index = 0, first_itd = 0;
			frame_index < urb->nPacketNum;
			frame_index++) {
		itd_real = kmalloc( sizeof( struct ehci_itd ) + 32, MEMF_KERNEL );
		if (!itd_real) {
			status = -ENOMEM;
			goto fail;
		}
		itd = ( struct ehci_itd* )( ((uint32)itd_real + 32 ) & ~31 );

		memset (itd, 0, sizeof *itd);
		itd->itd_real = itd_real;
...
itd_link (struct ehci_hcd *ehci, unsigned frame, struct ehci_itd *itd)
{
	...
	ehci->periodic [frame] = cpu_to_le32 (((uint32)itd)) | Q_TYPE_ITD;
}
...
static int get_iso_range (
	...
	struct list_head	*lh;
	struct hcd_dev		*dev = urb->psDevice->pHCPrivate;
	int			last = -1;
	unsigned		now, span, end;

	span = urb->nInterval * urb->nPacketNum;
	...
	list_for_each (lh, &dev->urb_list) {
		...
		u = list_entry (lh, USB_packet_s, lPacketList);
		if (u == urb || u->nPipe != urb->nPipe)
			continue;
		if (u->nInterval != urb->nInterval) {	/* must not change! */ 
			kerndbg(KERN_WARNING, "urb %p interval %d ... != %p interval %d\n",
				u, u->nInterval, urb, urb->nInterval);
			return -EINVAL;
		}
		
		/* URB for this endpoint... covers through when?  */
		itd = urb->pHCPrivate;
		s = itd->uframe + u->nInterval * u->nPacketNum;
		if (last < 0)
			last = s;
		else {
			/*
			 ...
			 */
			kerndbg(KERN_WARNING, "NYET: queue >2 URBs per ISO endpoint\n");
			return -EDOM;
		}
	}
	...
	if (!(urb->nTransferFlags & USB_ISO_ASAP)) {
		...
		urb->nStartFrame %= ehci->periodic_size;
		temp = urb->nStartFrame << 3;
		if (temp < *start)
			temp += mod;
		if (temp > *max)
			return -EDOM;
...
static int
itd_schedule (struct ehci_hcd *ehci, USB_packet_s *urb)
{
	...
	for (i = 0; i < urb->nPacketNum; i++) {
		urb->sISOFrame[i].nStatus = -EINPROGRESS;
	...
	do {
		 ...
		 */
		usecs /= urb->nPacketNum;
		usecs /= urb->nInterval;
		usecs >>= 3;
		if (usecs < 1)
			usecs = 1;
		g_psBus->claim_bandwidth (urb->psDevice, urb, usecs, 1);

		/* maybe enable periodic schedule processing */
		if (!ehci->periodic_sched++) {
			if ((status =  enable_periodic (ehci)) != 0) {
				// FIXME deschedule right away
				kerndbg( KERN_FATAL, "itd_schedule, enable = %d\n", status);
			}
		}
	...
	kerndbg( KERN_FATAL, "urb %p, CAN'T SCHEDULE\n", urb);
	return -ENOSPC;
}
...
itd_complete (
	...
	USB_packet_s				*urb = itd->urb;
	USB_ISO_frame_s	*desc;
	u32					t;

	/* update status for this uframe's transfers */
	desc = &urb->sISOFrame[itd->index];
	...
	if (t & EHCI_ISOC_ACTIVE)
		...
		if (t & EHCI_ISOC_BUF_ERR)
			desc->nStatus = usb_pipein (urb->nPipe)
				? -ENOSR  /* couldn't read */
				: -ECOMM; /* couldn't write */
		else if (t & EHCI_ISOC_BABBLE)
			desc->nStatus = -EOVERFLOW;
		else /* (t & EHCI_ISOC_XACTERR) */
			desc->nStatus = -EPROTO;

		/* HC need not update length with this error */
		if (!(t & EHCI_ISOC_BABBLE))
			desc->nActualLength += EHCI_ITD_LENGTH (t);
	} else {
		desc->nStatus = 0;
		desc->nActualLength += EHCI_ITD_LENGTH (t);
	}
...
static int itd_submit (struct ehci_hcd *ehci, USB_packet_s *urb, int mem_flags)
{
	int		status;
	unsigned long	flags;

	kerndbg( KERN_DEBUG, "itd_submit urb %p\n", urb);
...
static void
scan_periodic (struct ehci_hcd *ehci, SysCallRegs_s *regs)
{
...
restart:
		...
		while (q.ptr != 0) {
			...
			case Q_TYPE_FSTN:
				 ...
				 */
				if (q.fstn->hw_prev != EHCI_LIST_END) {
					kerndbg( KERN_DEBUG, "ignoring completions from FSTNs\n");
				}
...
#endif /* have_split_iso */
			default:
				kerndbg( KERN_DEBUG, "corrupt type %d frame %d shadow %p\n",
					(uint)type, frame, q.ptr);
				// BUG ();
				last = 1;
				q.ptr = 0;
			}
