...
#define	DEFAULT_I_TDPS		1024		/* some HCs can do less */
	...
	/* per root hub port */
	bigtime_t		reset_done [EHCI_MAX_ROOT_PORTS];
...
timer_action (struct ehci_hcd *ehci, enum ehci_timer_action action)
{
	if (!test_and_set_bit (action, &ehci->actions)) {
		...
		if (action != TIMER_IAA_WATCHDOG
				&& ( t + get_system_time() ) > ehci->watchdog_expires
				&& ehci->watchdog_pending == true )
			return;
		
		delete_timer( ehci->watchdog );
		ehci->watchdog_expires = get_system_time() + t;
		ehci->watchdog_pending = true;
		ehci->watchdog = create_timer();
		start_timer( ehci->watchdog, ehci_watchdog, ehci, t * 1000, true );
...
#define	QTD_STS_PING	(1 << 0)	/* issue PING? */
	...
	void*		qtd_real;		/* qtd real address */
	struct list_head	qtd_list;		/* sw qtd list */
	USB_packet_s		*urb;			/* qtd's urb */
...
#define	EHCI_ITD_IOC		(1 << 15)	/* interrupt on complete */
	...
	/* the rest is HCD-private */
	void*		itd_real;	/* for this itd */
	union ehci_shadow	itd_next;	/* ptr to periodic q entry */

	USB_packet_s		*urb;
	struct list_head	itd_list;	/* list of urb frames' itds */
	uint32		buf_dma;	/* frame's buffer address */
...
#define SUBMIT_URB(urb,mem_flags) g_psBus->submit_packet(urb)
#define STUB_DEBUG_FILES

static inline int hcd_register_root (struct usb_hcd *hcd)
{
	return g_psBus->new_device (hcd_to_bus (hcd)->psRootHUB);
}
