...
#define usb_to_ohci(usb)	((struct ohci_device *)(usb)->pHCPrivate)

/* hcd */
/* endpoint */
static int ep_link(ohci_t * ohci, ed_t * ed);
static int ep_unlink(ohci_t * ohci, ed_t * ed);
static ed_t * ep_add_ed(USB_device_s * usb_dev, unsigned int pipe, int interval, int load, int mem_flags);
static void ep_rm_ed(USB_device_s * usb_dev, ed_t * ed);
/* td */
static void td_fill(ohci_t * ohci, unsigned int info, dma_addr_t data, int len, USB_packet_s * urb, int index);
...
#define ALLOC_FLAGS MEMF_KERNEL
...
dma_to_ed_td (struct hash_list_t * entry, dma_addr_t dma)
{
	...
	while (scan && scan->dma != dma)
		scan = scan->next;
	if (!scan)
		kerndbg( KERN_FATAL, "ERROR: dma_to_ed_tt()\n" );
	return scan->virt;
}
...
td_alloc (struct ohci *hc, int mem_flags)
{
	...
	real = kmalloc( sizeof( struct td ) + 32, MEMF_KERNEL | MEMF_CLEAR );
	td = ( struct td* )( ((uint32)real + 32 ) & ~31 );
...
dev_alloc (struct ohci *hc, int mem_flags)
{
	...
	real = kmalloc( sizeof( struct ohci_device ) + 16, MEMF_KERNEL | MEMF_CLEAR );
	dev = ( struct ohci_device* )( ((uint32)real + 16 ) & ~15 );


	//dev = pci_pool_alloc (hc->dev_cache, mem_flags, &dma);
	if (dev) {
		//memset (dev, 0, sizeof (*dev));
		dev->dma = dma = (uint32)dev;
		dev->real = real;
