...
#define HDA_MAX_CONNECTIONS 50

typedef struct _HDANode
{
	int nNode;
	bool bSearched;
	bool bConfigured;
	...
	uint32 nAmpInCaps;
	uint32 nConns;
	struct _HDANode* pasConns[HDA_MAX_CONNECTIONS];
} HDANode_s;

#define HDA_MAX_OUTPUT_PATHS 10

typedef struct _HDAOutputPath
{
	int nDacNode;
	int nVolNode;
	int nVolIndex;
...
#define HDA_NAME_LENGTH 255

typedef struct
{
	int nDeviceID;
	int nDeviceHandle;
	char zName[HDA_NAME_LENGTH];
	bool bSPDIF;
	bool bRecord;
	uint32 nFormats;
	int nMaxChannels;
