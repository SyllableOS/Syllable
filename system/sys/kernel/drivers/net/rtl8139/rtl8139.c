...
static int32 rtl8129_timer(void* data);
static void rtl8129_tx_timeout(struct device *dev);
static void rtl8129_init_ring(struct device *dev);
static int rtl8129_start_xmit(PacketBuf_s* skb, struct device *dev);
static int rtl8129_rx(struct device *dev);
static int rtl8129_interrupt( int irq, void* data, SysCallRegs_s* regs );
...
//	dev = init_etherdev(dev, 0);
	...
	if (read_eeprom(ioaddr, 0) != 0xffff)
		for (i = 0; i < 3; i++)
			((uint16 *)(dev->dev_addr))[i] = read_eeprom(ioaddr, i + 7);
	else
		for (i = 0; i < 6; i++)
...
//	tp = kmalloc(sizeof(*tp), GFP_KERNEL | GFP_DMA);
	tp = kmalloc(sizeof(*tp), MEMF_KERNEL );
	memset(tp, 0, sizeof(*tp));
	dev->priv = tp;

	spinlock_init( &tp->tx_spinlock, "rtl8139_tx_spinlock" );
...
void rtl8129_wakup_tx( struct device* dev )
{
...
int rtl8129_wait_for_tx( struct device* dev )
{
	struct rtl8129_private *tp = (struct rtl8129_private *)dev->priv;
//	printk( "s\n" );
...
rtl8129_open(struct device *dev)
{
	...
	tp->tx_sema = create_semaphore( "rtl8192_tx_sem", 0, 0 );
	dev->irq_handle = request_irq( dev->irq, &rtl8129_interrupt, NULL, SA_SHIRQ, "ne2000_device", dev );
	
	if ( dev->irq_handle < 0 ) {
		return -EAGAIN;
	}

	tp->tx_bufs = kmalloc(TX_BUF_SIZE * NUM_TX_DESC, MEMF_KERNEL);
	tp->rx_ring = kmalloc(RX_BUF_LEN + 16, MEMF_KERNEL);
...
static void rtl8129_tx_timeout(struct device *dev)
{
	...
	for (mii_reg = 0; mii_reg < 8; mii_reg++)
		printk(" %4.4x\n", mdio_read(dev, tp->phys[0], mii_reg));
	printk(".\n");
	...
	{							/* Save the unsent Tx packets. */
		PacketBuf_s *saved_skb[NUM_TX_DESC], *skb;
		int j;
		...
		for (i = 0; i < j; i++) {
			...
			if ((long)skb->pb_pData & 3) {		/* Must use alignment buffer. */
				memcpy(tp->tx_buf[i], skb->pb_pData, skb->pb_nSize);
				outl(virt_to_bus(tp->tx_buf[i]), ioaddr + TxAddr0 + i*4);
			} else
				outl(virt_to_bus(skb->pb_pData), ioaddr + TxAddr0 + i*4);
...
static int
rtl8129_start_xmit( PacketBuf_s *skb, struct device *dev)
{
	struct rtl8129_private *tp = (struct rtl8129_private *)dev->priv;
	long ioaddr = dev->base_addr;
	int entry;
...
again:
	...
	if (test_and_set_bit(0, (void*)&dev->tbusy) != 0) {
		if ( spinunlock_and_suspend( tp->tx_sema, &tp->tx_spinlock, flags, 4000000LL ) < 0 ) {
			printk( "Error: rtl8129_wait_for_tx() timed out\n" );
			return 1;
		}
...
static int rtl8129_rx(struct device *dev)
{
	...
	while ((inb(ioaddr + ChipCmd) & 1) == 0) {
		...
		if (rx_status & RxTooLong) {
			...
			skb = alloc_pkt_buffer(rx_size + 2);
			if (skb == NULL) {
...
static status_t rtl8129_ioctl( void* pNode, void* pCookie, uint32 nCommand, void* pArgs, bool bFromKernel )
{
    ...
    switch( nCommand )
    {
		case SIOC_ETH_START:
		{
			...
			psDev->timer_thread = spawn_kernel_thread( "rtl8129_timer", rtl8129_timer, NORMAL_PRIORITY, 0, psDev );
