...
int emu10k1_interrupt(int irq, void *pdata, SysCallRegs_s* regs)
{
	struct emu10k1_card* card = (struct emu10k1_card*) pdata;
	uint32 irqstatus, irqstatus_tmp;
	...
	while ((irqstatus = inl(card->iobase + IPR))) {
		...
		if (irqstatus){
			printk("emu10k1: Warning, unhandled interrupt: %#08X\n", (int)irqstatus);
