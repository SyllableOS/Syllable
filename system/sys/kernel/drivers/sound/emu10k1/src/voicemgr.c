 ...
 *
 */
int emu10k1_voice_alloc_buffer(struct emu10k1_card *card, struct voice_mem *mem, uint32 pages)
{
	uint32 pageindex, pagecount;
...
//		if ((mem->addr[pagecount] = (void*)PAGE_ALIGN((uint32)(alloc_dma_mem(card->pci_dev, PAGE_SIZE*2, &mem->dma_handle[pagecount]))))
		if ((mem->addr[pagecount] = alloc_dma_mem(card->pci_dev, PAGE_SIZE, &mem->dma_handle[pagecount]))
			== NULL) {
			mem->pages = pagecount;
...
//			mem->dma_handle[pagecount] = mem->addr[pagecount];

		DPD(2, "Virtual Addx: %p\n", mem->addr[pagecount]);

//		printk("virtpage: %ld, Virtual Addx: %p\n", pagecount, mem->addr[pagecount]);

		for (i = 0; i < PAGE_SIZE / EMUPAGESIZE; i++) {
			busaddx = (uint32)(mem->dma_handle[pagecount]) + i * EMUPAGESIZE;

			DPD(3, "Bus Addx: %#lx\n", busaddx);
//			printk( "Bus Addx: %#lx = dma handle: %lx\n", busaddx, mem->dma_handle[pagecount]);

			pageindex = mem->emupageindex + pagecount * PAGE_SIZE / EMUPAGESIZE + i;

			((uint32 *) card->virtualpagetable.addr)[pageindex] = (uint32)((busaddx * 2) | pageindex);
			
//			printk("card page %ld = addr %lx\n", pageindex, (uint32)((busaddx * 2) | pageindex));
		}
	}
...
void emu10k1_voice_free_buffer(struct emu10k1_card *card, struct voice_mem *mem)
{
	...
	for (pagecount = 0; pagecount < mem->pages; pagecount++) {
		free_dma_mem(card->pci_dev, PAGE_SIZE,
					mem->addr[pagecount],
					mem->dma_handle[pagecount]);

		for (i = 0; i < PAGE_SIZE / EMUPAGESIZE; i++) {
			pageindex = mem->emupageindex + pagecount * PAGE_SIZE / EMUPAGESIZE + i;
			((uint32 *) card->virtualpagetable.addr)[pageindex] =
				(uint32)(((uint32)card->silentpage.dma_handle * 2) | pageindex);
		}
	}
...
{
	uint8 *voicetable = card->voicetable;
	int i;
	unsigned long flags;
