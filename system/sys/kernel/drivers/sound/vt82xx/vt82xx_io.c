...
inline void sg_begin (struct via_channel *chan)
{
	outb (VIA_SGD_START | VIA_SGD_FLAG | VIA_SGD_EOL 
		| VIA_SGD_STOP | VIA_SGD_AUTOSTART, chan->iobase + VIA_PCM_CONTROL);
}
...
void via_chan_init_defaults (struct via_info *card, struct via_channel *chan)
{
	...
	if (chan == &card->ch_out) {
		chan->pcm_fmt = VIA_PCM_FMT_VT_MONO;
		chan->pcm_stop = VIA_PCM_STOP_VT_MONO | VIA_PCM_STOP_NEVER;
	}
	else {
		chan->pcm_fmt = 0;
		chan->pcm_stop = VIA_PCM_STOP_NEVER;
	}
	chan->is_enabled = 1;
 ...
 void via_chan_pcm_fmt (struct via_channel *chan, int reset)
{
	...
	if (reset) {
		...
		if( chan->is_record ) {
			...
			chan->pcm_fmt = VIA_PCM_FMT_VT_MONO;
			chan->pcm_stop = VIA_PCM_STOP_VT_MONO;
		}
	}
	chan->pcm_stop |= VIA_PCM_STOP_NEVER;
	if (!chan->is_record) {
	...
	outl (chan->pcm_stop, chan->iobase + VIA_PCM_STOP);
	if( chan->is_record ) {
		outb( VIA_PCM_CAPTURE_FIFO_ENABLE, chan->iobase + VIA_PCM_CAPTURE_FIFO );
	}
			     ...
			     struct via_channel *chan, int val)
{
	...
	case AFMT_S16_LE:
		if (!chan->is_record) {
			if ((chan->pcm_fmt & VIA_PCM_FMT_VT_16BIT) == 0) {
				chan->pcm_fmt |= VIA_PCM_FMT_VT_16BIT;
				via_chan_pcm_fmt (chan, 0);
			}
		}
		else {
			if ((chan->pcm_stop & VIA_PCM_STOP_16BIT) == 0) {
				chan->pcm_stop |= VIA_PCM_STOP_16BIT;
				via_chan_pcm_fmt (chan, 0);
			}
		}
		break;
	...
	case AFMT_U8:
		if (!chan->is_record) {
			if ((chan->pcm_fmt & VIA_PCM_FMT_VT_16BIT)) {
				chan->pcm_fmt &= ~VIA_PCM_FMT_VT_16BIT;
				via_chan_pcm_fmt (chan, 0);
			}
		}
		else {
			if ((chan->pcm_stop & VIA_PCM_STOP_16BIT)) {
				chan->pcm_stop &= ~VIA_PCM_STOP_16BIT;
				via_chan_pcm_fmt (chan, 0);
			}
		}
			        ...
			        struct via_channel *chan, int val)
{
	...
	case 1:
		if (chan->is_record) {
			chan->pcm_stop &= ~VIA_PCM_STOP_STEREO;
		} else {
			chan->pcm_fmt &= ~VIA_PCM_FMT_VT_STEREO;
			chan->pcm_fmt |= VIA_PCM_FMT_VT_MONO;
			chan->pcm_stop = VIA_PCM_STOP_VT_MONO | VIA_PCM_STOP_NEVER;
		}
		via_chan_pcm_fmt (chan, 0);
	...
	case 2:
		if (chan->is_record) {
			...
			chan->pcm_fmt &= ~VIA_PCM_FMT_VT_MONO;
			chan->pcm_fmt |= VIA_PCM_FMT_VT_STEREO;
			chan->pcm_stop = VIA_PCM_STOP_VT_STEREO | VIA_PCM_STOP_NEVER;
		}
		via_chan_pcm_fmt (chan, 0);
		break;
	...
	if (val < 0) {
		...
		if (chan->is_record) {
			chan->frag_size = (VIA_DEFAULT_FRAG_TIME * chan->rate *
				   	((chan->pcm_stop & VIA_PCM_STOP_STEREO) ? 2 : 1) *
				   ((chan->pcm_stop & VIA_PCM_STOP_16BIT) ? 2 : 1)) / 1000 - 1;
		} else {
			chan->frag_size = (VIA_DEFAULT_FRAG_TIME * chan->rate *
				   ((chan->pcm_fmt & VIA_PCM_FMT_VT_STEREO) ? 2 : 1) *
				   ((chan->pcm_fmt & VIA_PCM_FMT_VT_16BIT) ? 2 : 1)) / 1000 - 1;
		}
 ...
 u8 via_ac97_wait_idle (struct via_info *card)
{
	...
	do {
		...
		tmp8 = inb (card->baseaddr + VIA_BASE0_AC97_CTRL);
	} while ((tmp8 & VIA_CR80_BUSY) && (counter-- > 0));

	if (tmp8 & VIA_CR80_BUSY)
 ...
 void via_ac97_write_reg (struct ac97_codec *codec, u8 reg, u16 value)
{
	...
	for (counter = VIA_COUNTER_LIMIT; counter > 0; counter--) {
		if ((inb (card->baseaddr + VIA_BASE0_AC97_CTRL) & VIA_CR80_BUSY) == 0)
			goto out;
		udelay(15);
	}
