	...
	for ( i = 0 ; i < psStream->ds_sDoubleIndirect.len ; ++i )
	{
		...
		for ( k = 0 ; k < pasIndirectBlock[j].len ; ++k )
		{
			...
			if ( GETBIT( pBitMap, nDirectBlock + k ) ) {
			    printk( "PANIC: Direct pointer block in double-indirect area %d (%d) is double bucked!",
				    k, (int)nDirectBlock + k );
			}
...
static int afs_alloc_meta_blocks( AfsVolume_s* psVolume, BlockRun_s* psRun,
				  BlockRun_s* psPrevRun, int nGroup, int nSize )
{
    off_t nBlkNum;
    int   nError;
    int   i;
...
static status_t afs_alloc_data_blocks( AfsVolume_s* psVolume, AfsInode_s* psInode, BlockRun_s* psNewRun, BlockRun_s* psPrevRun,
				       int nDataGroup, int nSize )
{
    status_t nError;
...
static off_t afs_expand_indirect_range( AfsVolume_s* psVolume, AfsInode_s* psInode, int nDeltaSize, bool* pbDiskFull )
{
	...
	for ( j = 0 ; j < nPtrsPerBlk && nSize > 0 ; ++j ) {
		...
		if ( psBlock[j].group == sNewRun.group &&  (psBlock[j].start + psBlock[j].len) == sNewRun.start ) {
			...
			if ( nUsed > AFS_MAX_RUN_LENGTH ) nUsed = AFS_MAX_RUN_LENGTH;
		    } else {
			nUsed = sNewRun.len;
		    }
...
static off_t afs_expand_doubleindirect_range( AfsVolume_s* psVolume, AfsInode_s* psInode, off_t nDeltaSize )
{
    ...
    const off_t nCurPos = (psStream->ds_nMaxDoubleIndirectRange > 0) ?
	psStream->ds_nMaxDoubleIndirectRange - psStream->ds_nMaxIndirectRange : 0;
	...
	nError = afs_alloc_meta_blocks( psVolume, &psStream->ds_sDoubleIndirect, NULL,
					psInode->ai_sInodeNum.group, BLOCKS_PER_DI_RUN );
...
//    printk( "Alloc double indirect block %d\n", afs_run_to_num( psSuperBlock, &psStream->ds_sDoubleIndirect ) );
	...
	for ( ; nIDPtr < nPtrsPerBlk && nSize > 0 && nError >= 0 ; ++nIDPtr ) {
		...
		nError = afs_alloc_meta_blocks( psVolume, &psIndirectBlock[nIDPtr],
						((nIDPtr > 0) ? &psIndirectBlock[nIDPtr - 1] : NULL),
						psInode->ai_sInodeNum.group, BLOCKS_PER_DI_RUN );
		if ( nError < 0 ) {
		    break;
		}
...
//  printk( "Expand stream with %d blocks\n", (int) nDeltaSize );
	...
	for ( i = 0 ; i < DIRECT_BLOCK_COUNT && nSize > 0 ; ++i ) {
	    ...
	    if ( sNewRun.len == 0 ) {
		int j;
		nError = afs_alloc_blocks( psVolume, &sNewRun,
					   ((psStream->ds_asDirect[i].len > 0) ? &psStream->ds_asDirect[i] : NULL), nDataGroup, nSize, 1 );
		if ( nError < 0 ) {
	    ...
	    if ( psStream->ds_asDirect[i].len > 0 ) {
		if ( psStream->ds_asDirect[i].group == sNewRun.group &&
		     (psStream->ds_asDirect[i].start + psStream->ds_asDirect[i].len) == sNewRun.start ) {
		    uint nUsed;
...
int afs_trunc_indirect( AfsVolume_s* psVolume, DataStream_s* psStream, off_t nDeltaSize )
{
    ...
    if ( nDeltaSize == 0 ) { // Seems like someting whent wrong during an allocation.
	if ( psStream->ds_sIndirect.len == 0 ) {
    ...
    if ( nDeltaSize != 0 ) {
	printk( "%Ld, %Ld, %Ld, %Ld (%Ld)\n", psStream->ds_nMaxDirectRange, psStream->ds_nMaxIndirectRange,
		psStream->ds_nMaxDoubleIndirectRange, psStream->ds_nSize, nOrigDelta );
...
//    printk( "afs_truncate_file() truncate file from %d to %d\n", (int)nOldSize, (int)nNewSize );
	...
	off_t nDeltaSize =
	    (nNewSize > psStream->ds_nMaxIndirectRange) ?
	    (psStream->ds_nMaxDoubleIndirectRange - nNewSize) :
	    (psStream->ds_nMaxDoubleIndirectRange - psStream->ds_nMaxIndirectRange);
	...
	off_t nDeltaSize =
	    (nNewSize > psStream->ds_nMaxDirectRange) ?
	    (psStream->ds_nMaxIndirectRange - nNewSize) :
	    (psStream->ds_nMaxIndirectRange - psStream->ds_nMaxDirectRange);
...
int afs_get_stream_blocks( AfsVolume_s* psVolume, DataStream_s* psStream,
			   off_t nPos, int nRequestLen, off_t* pnStart, int* pnActualLength )
{
    AfsSuperBlock_s* psSuperBlock = psVolume->av_psSuperBlock;
