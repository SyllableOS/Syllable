...
int bt_end_transaction( AfsVolume_s* psVolume, AfsInode_s* psInode, BTransaction_s* psTrans, bool bOkToWrite )
{
	...
	{
	    off_t nDeltaSize =
		((psTree->bt_nLastNode + nBlockSize - 1) / nBlockSize) - afs_get_inode_block_count( psInode );
...
error:
	...
	if ( psTrans->bt_apsLockedNodes[i].be_bDirty ) {
	    if ( bOkToWrite ) {
		nError = afs_do_write( psVolume, psInode, (char*) psTrans->bt_apsLockedNodes[i].be_psNode,
				       psTrans->bt_apsLockedNodes[i].be_nValue, B_NODE_SIZE );
		if ( nError <  0 ) {
...
status_t bt_append_key( BNode_s* psNode, const void* pKey, int nKeySize, bvalue_t nValue )
{
    ...
    memmove( ((char*)B_KEY_VALUE_OFFSET( psNode )) + nExtraSize + B_SIZEOF_KEYOFFSET,
	     B_KEY_VALUE_OFFSET( psNode ), psNode->bn_nKeyCount * B_SIZEOF_VALUE );
    memmove( ((char*)B_KEY_INDEX_OFFSET( psNode )) + nExtraSize, B_KEY_INDEX_OFFSET( psNode ),
	     psNode->bn_nKeyCount * B_SIZEOF_KEYOFFSET );
	...
	kassertw( B_KEY_INDEX_OFFSET( psNode )[ psNode->bn_nKeyCount - 1 ] >
		  B_KEY_INDEX_OFFSET( psNode )[ psNode->bn_nKeyCount - 2 ] );
    }
...
status_t bt_insert_key_to_node( AfsInode_s* psInode, BNode_s* psNode, const void* a_pKey, int a_nKeySize, bvalue_t nValue )
{
	...
	if ( bInserted == false ) {
		...
		memmove( &psNode->bn_anKeyData[nKeyOffset + a_nKeySize ], &psNode->bn_anKeyData[nKeyOffset],
			 psNode->bn_nTotKeySize - nKeyOffset );
		 ...
		 const void* pKey, int nKeySize, BNode_s** ppsNode )
{
	...
	for ( i = 0 ; i <= psRoot->bn_nKeyCount ; ++i ) {
		...
		if ( false == bIsLeaf ) { // Check if we got to a leaf node
		    psStack->bn_apnStack[ psStack->bn_nCurPos ] = nRoot;
		    psStack->bn_apsNodes[ psStack->bn_nCurPos ] = psRoot;
...
static bool bt_merge_node( AfsInode_s* psInode, BNode_s* psDstNode, const BNode_s* psSrcNode,
			   int* pnStartIndex, int* pnStartOffset, int nMaxSize,
			   const void* pKey, int nKeyLen, bvalue_t psValue, bool bInserted )
{
    int	    	    nKeyOffset;
	...
	if ( false == bInserted &&
	     bt_cmp_keys( psInode->ai_nIndexType, pKey, nKeyLen,
			  &psSrcNode->bn_anKeyData[nKeyOffset], nCurKeySize ) < 0 )
	{
...
status_t bt_add_key_to_node( AfsVolume_s* psVolume, AfsInode_s* psInode, BTransaction_s* psTrans, BStack_s* psStack,
			     BNode_s* psNode, bvalue_t nNode, const void* pKey, int nKeyLen, bvalue_t psValue )
{
    int	nStackLevel;
    status_t nError = 0;
	...
	if ( bt_will_key_fit( psNode, nKeyLen ) ) {
	    ...
	    i = 0;
	    bInserted = bt_merge_node( psInode, psNewNode1, psNode, &i, &nKeyOffset,
				       ((B_NODE_SIZE - B_HEADER_SIZE) / 2), pKey, nKeyLen, psValue, bInserted );
	    ...
	    if ( false == bInserted &&
		 bt_cmp_keys( psInode->ai_nIndexType, pKey, nKeyLen, &psNode->bn_anKeyData[nKeyOffset], nParentKeyLen ) < 0 ) {
		pParentKey 		 = pKey;
...
//				printk( "Parent key = %d\n", *((int*)pParentKey) );
	    ...
	    bInserted = bt_merge_node( psInode, psNewNode2, psNode, &i, &nKeyOffset, B_NODE_SIZE * 2,
				       pKey, nKeyLen, psValue, bInserted );
...
//		printk( " " );
	...
	for ( i = 0 ; i < psRoot->bn_nKeyCount ; ++i )
	{
		...
		bDidPrint = _bt_print_tree( psVolume, psInode, psStack,
					    bt_load_node( psVolume, psInode, psStack, apsValues[i] ), nLevel - 1 ) || bDidPrint;
...
//					printk( "-" );
	...
	if ( NULL_VAL != psRoot->bn_nOverflow ) {
	    bDidPrint = _bt_print_tree( psVolume, psInode, psStack,
					bt_load_node( psVolume, psInode, psStack, psRoot->bn_nOverflow ), nLevel - 1 ) || bDidPrint;
	    printk( "-" );
	}
	return( bDidPrint );
    }
...
static int bt_find_linear( AfsVolume_s* psVolume, AfsInode_s* psInode,
		   ...
		   const void* pKey, int nKeySize, bvalue_t nValue )
{
	...
	i = 0;
	bInserted = bt_merge_node( psInode, psNewNode1, psNode, &i, &nKeyOffset, (B_NODE_SIZE - B_HEADER_SIZE) / 2,
				   pKey, nKeySize, nValue, bInserted );
	...
	if ( psStack->bn_nCurPos > 0  ) {
	    ...
	    nError = bt_add_key_to_node( psVolume, psInode, psTrans, psStack, psParent, nParent, &psNewNode2->bn_anKeyData[0],
					 B_KEY_INDEX_OFFSET( psNewNode2 )[0], nNewNode1 );
	}
	else
	{
		   ...
		   const void* pKey, int nKeySize, BIterator_s* psIterator )
{
	...
	if ( psIterator->bi_nCurKey < sStack.bn_apsNodes[ sStack.bn_nCurPos ]->bn_nKeyCount ) {
	    bt_get_key( sStack.bn_apsNodes[ sStack.bn_nCurPos ],
			psIterator->bi_nCurKey, &pKey, &psIterator->bi_nKeySize );
	    memcpy( psIterator->bi_anKey, pKey, psIterator->bi_nKeySize );

	    psIterator->bi_nCurValue =
		B_KEY_VALUE_OFFSET( sStack.bn_apsNodes[ sStack.bn_nCurPos ] )[ psIterator->bi_nCurKey ];
	}
    }
error:
