...
static inline off_t __afs_run_to_num( const AfsSuperBlock_s* psSuperBlock, const BlockRun_s* psRun,
				      const char* pzFile, const char* pzFunction, int nLine )
{
...
int afs_create_inode( AfsVolume_s* psVolume, AfsInode_s* psParent, int nMode, int nFlags, int nIndexType,
		      const char* pzName, int nNameLen, AfsInode_s**ppsRes ); // ino_t* pnInodeNum );
