...
ISOReadDirEnt(nspace* volume, dircookie* cookie, struct kernel_dirent* buf, size_t bufsize)
{
	...
	if (blockData != NULL && totalRead < cookie->totalSize)
	{
		...
		if ((((result = InitNode( volume, &node, blockData + cookie->pos, &bytesRead, volume->joliet_level))) == 
							-EOK))
		{
...
int ISO_HIDDEN
InitNode(nspace * volume, vnode* rec, char* buf, int* bytesRead, uint8 joliet_level)
{
 int 	result = -EOK;
 ...
 if (recLen > 0)
 {
  ...
  if (rec->fileIDLen > 0)
  {
   if(rec->fileIDLen == 1)
   {
    ...
    {
     int i,err;
     err = nls_conv_cp_to_utf8(
			NLS_UTF16_BE,
			buf,
			rec->fileIDString,
			rec->fileIDLen,
			ISO_MAX_FILENAME_LENGTH);
     if(err < 0) {
