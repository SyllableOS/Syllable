...
uint8 *ntfs_find_attr_in_mft_rec( ntfs_volume_s * vol, uint8 *m, uint32 type, wchar_t *name, uint32 name_len, int ic, uint16 instance )
{
	...
	for( ; a >= m && a <= m + vol->mft_record_size; a += NTFS_GETU32( a + 4 /* length */  ) )
	{
		...
		if( name && !ntfs_are_names_equal( name, name_len, ( wchar_t * )( a + NTFS_GETU16( a + 10 /* name_offset */  ) ), a[9] /* name_length */ , ic, ( wchar_t * )vol->upcase, vol->upcase_length ) )
		{
			register int rc;

			rc = ntfs_collate_names( ( wchar_t * )vol->upcase, vol->upcase_length, name, name_len, ( wchar_t * )( a + NTFS_GETU16( a + 10 /* name_offset */  ) ), a[9] /* name_length */ , 1, 1 );
			/*
			...
			if( rc )
				continue;
			rc = ntfs_collate_names( ( wchar_t * )vol->upcase, vol->upcase_length, name, name_len, ( wchar_t * )( a + NTFS_GETU16( a + 10 /* name_offset */  ) ), a[9] /* name_length */ , 0, 1 );
			if( rc == -1 )
				return NULL;
			if( rc )
				continue;
	...
	ntfs_error( "ntfs_find_attr_in_mft_record: mft record 0x%p is corrupt. Run chkdsk.\n", m );
...
int ntfs_new_attr( ntfs_inode_s * ino, int type, void *name, int namelen, void *value, int value_len, int *pos, int *found )
{
	...
	for( i = 0; i < ino->attr_count; i++ )
	{
		...
		/* If @name is present, compare the two names. */
		if( namelen && !ntfs_are_names_equal( ( wchar_t * )name, namelen, ( wchar_t * )a->name, a->namelen /* name_length */ , 1 /* ignore case */ , ( wchar_t * )ino->vol->upcase, ino->vol->upcase_length ) )
		{
			register int rc;

			rc = ntfs_collate_names( ( wchar_t * )ino->vol->upcase, ino->vol->upcase_length, ( wchar_t * )a->name, a->namelen, ( wchar_t * )name, namelen, 1 /* ignore case */ , 1 );
			if( rc == -1 )
				continue;
			if( rc == 1 )
			{
			...
			rc = ntfs_collate_names( ( wchar_t * )ino->vol->upcase, ino->vol->upcase_length, ( wchar_t * )a->name, a->namelen, ( wchar_t * )name, namelen, 0 /* case sensitive */ , 1 );
			if( rc == -1 )
				continue;
			if( rc == 1 )
			{
		...
		if( value_len && a->resident )
		{
			if( !a->resident )
			{
				ntfs_error( "ntfs_new_attr: Value specified but " "attribute non-resident. Bug!\n" );
				return -EINVAL;
			}
			m = value_len;
			if( m > a->size )
	...
	if( ino->attr_count % 8 == 0 )
	{
		...
		new = ( ntfs_attribute * ) ntfs_malloc( ( ino->attr_count + 8 ) * sizeof( ntfs_attribute ) );
		if( !new )
			return -ENOMEM;
		if( ino->attrs )
		{
	...
	if( do_insert )
		ntfs_memmove( ino->attrs + i + 1, ino->attrs + i, ( ino->attr_count - i ) * sizeof( ntfs_attribute ) );
	ino->attr_count++;
...
int ntfs_make_attr_resident( ntfs_inode_s * ino, ntfs_attribute * attr )
{
	int64 size = attr->size;
...
int ntfs_insert_run( ntfs_attribute * attr, int cnum, off_t cluster, int len )
{
	...
	if( ( attr->d.r.len * sizeof( ntfs_runlist ) ) % PAGE_SIZE == 0 )
	{
		...
		if( !new )
		{
			ntfs_error( "ntfs_insert_run: ntfs_malloc(new_size = 0x%lx) failed\n", new_size );
			return -1;
		}
		if( attr->d.r.runlist )
		{
			ntfs_memcpy( new, attr->d.r.runlist, attr->d.r.len * sizeof( ntfs_runlist ) );
			ntfs_free( attr->d.r.runlist );
		}
		attr->d.r.runlist = new;
	}
	if( attr->d.r.len > cnum )
		ntfs_memmove( attr->d.r.runlist + cnum + 1, attr->d.r.runlist + cnum, ( attr->d.r.len - cnum ) * sizeof( ntfs_runlist ) );
...
int ntfs_extend_attr( ntfs_inode_s * ino, ntfs_attribute * attr, const int64 len )
{
	...
	if( ( attr->flags & ( ATTR_IS_COMPRESSED | ATTR_IS_ENCRYPTED ) ) || ino->record_count > 1 )
		return -EOPNOTSUPP;
	/*
	 ...
	 */
	clen = ( len - attr->allocated + ino->vol->cluster_size - 1 ) >> ino->vol->cluster_size_bits;
	if( !clen )
		return 0;
	err = ntfs_allocate_clusters( ino->vol, &cluster, &clen, &rl2, &rl2_len, DATA_ZONE );
	if( err )
		return err;
	attr->allocated += ( int64 )clen << ino->vol->cluster_size_bits;
	if( rlen > 0 )
	{
...
int ntfs_attr_allnonresident( ntfs_inode_s * ino )
{
	...
	{
		if( ino->attrs[i].type != vol->at_security_descriptor && ino->attrs[i].type != vol->at_data )
			continue;
...
int ntfs_resize_attr( ntfs_inode_s * ino, ntfs_attribute * attr, int64 newsize )
{
	...
	if( attr->resident )
	{
		...
		if( newsize )
		{
			...
			{
				minsize = oldsize;
				ntfs_bzero( ( char * )attr->d.data + oldsize, newsize - oldsize );
			}
	...
	if( newsize < oldsize )
	{
		...
		{
			if( ( int64 )( count + rl[i].len ) << clustersizebits > newsize )
			{
				i++;
				break;
			}
		...
		if( rl[--i].lcn != ( off_t )-1 )
		{
			off_t rounded = newsize - ( ( int64 )count << clustersizebits );

			rounded = ( rounded + ino->vol->cluster_size - 1 ) >> clustersizebits;
			error = ntfs_deallocate_cluster_run( ino->vol, rl[i].lcn + rounded, rl[i].len - rounded );
			if( error )
		...
		/* Free all other runs. */
		i++;
		error = ntfs_deallocate_clusters( ino->vol, rl + i, attr->d.r.len - i );
		if( error )
			return error;	/* FIXME: Incomplete operation. */
		/*
		 ...
		 */
		rl_size = ( ( i + 1 ) * sizeof( ntfs_runlist ) + PAGE_SIZE - 1 ) & PAGE_MASK;
		if( rl_size < ( ( attr->d.r.len * sizeof( ntfs_runlist ) + PAGE_SIZE - 1 ) & PAGE_MASK ) )
		{
	...
	else
	{
		...
		if( error )
			return error;	/* FIXME: Incomplete operation. */
		newcount = ( newsize + ino->vol->cluster_size - 1 ) >> clustersizebits;
	}
	/* Fill in new sizes. */
	attr->allocated = ( int64 )newcount << clustersizebits;
	attr->size = newsize;
...
int ntfs_create_attr( ntfs_inode_s * ino, int anum, char *aname, void *data, int dsize, ntfs_attribute ** rattr )
{
	...
	error = ntfs_new_attr( ino, anum, name, namelen, data, dsize, &i, &found );
	if( error || found )
	{
		ntfs_free( name );
	...
	*rattr = attr = ino->attrs + i;
	...
	if( attr->attrno + 1 != NTFS_GETU16( ino->attr + 0x28 ) )
		ntfs_error( "UH OH! attr->attrno (%i) != NTFS_GETU16(ino->attr + 0x28) (%i)\n", attr->attrno, NTFS_GETU16( ino->attr + 0x28 ) );
	attr->resident = 1;
	attr->flags = 0;
	attr->cengine = 0;
 ...
 * attribute.
 */
static int ntfs_process_runs( ntfs_inode_s * ino, ntfs_attribute * attr, unsigned char *data )
{
	int startvcn, endvcn;
	int vcn, cnum;
	off_t cluster;
	...
	if( !endvcn )
	{
		...
		else
		{
			 ...
			 * error. */
			ntfs_error( "Corrupt attribute extent. (endvcn is missing)\n" );
			return -1;
		}
	}
...
int ntfs_insert_attribute( ntfs_inode_s * ino, unsigned char *attrdata )
{
	...
	if( NTFS_GETU8( attrdata + 8 ) == 0 )
		error = ntfs_new_attr( ino, type, name, namelen, attrdata + NTFS_GETU16( attrdata + 0x14 ), NTFS_GETU16( attrdata + 0x10 ), &i, &found );
	else
		error = ntfs_new_attr( ino, type, name, namelen, NULL, 0, &i, &found );
	if( error )
	{
		if( name )
			ntfs_free( name );
	...
	attr->flags = *( uint16 * )( attrdata + 0xC );
...
int ntfs_read_compressed( ntfs_inode_s * ino, ntfs_attribute * attr, int64 offset, ntfs_io * dest )
{
	...
	int64 copied, len, chunk, offs1, l, chunk2;
	off_t cluster, cl1;
	char *comp = 0, *comp1;
	char *decomp = 0;
	ntfs_io io;
	...
	while( l )
	{
		...
		if( cluster == ( off_t )-1 )
		{
			...
			ll = ( ( int64 )( vcn + len ) << clustersizebits ) - offset;
			if( ll > l )
				ll = l;
			chunk = ll;
		...
		else if( dest->do_read )
		{
			...
			do
			{
				...
				if( l1 > 16 - got )
					l1 = 16 - got;
				io.size = ( int64 )l1 << clustersizebits;
				...
				got += l1;
				comp1 += ( int64 )l1 << clustersizebits;
			}
			while( cluster != ( off_t )-1 && got < 16 );
			/* Until empty run. */
			chunk = 16 << clustersizebits;
			if( cluster != ( off_t )-1 || got == 16 )
				/* Uncompressible */
				comp1 = comp;
			else
			{
				if( !decomp )
				{
					decomp = ntfs_malloc( 16 << clustersizebits );
					if( !decomp )
					{
						error = -ENOMEM;
						goto out;
			...
			offs1 = offset - ( ( int64 )s_vcn << clustersizebits );
