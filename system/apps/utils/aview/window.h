...
enum Menus{ID_FILE_LOAD,ID_VOID,ID_EXIT};
...
AppWindow::AppWindow(const Rect& cFrame) : Window(cFrame, "main_window", "AView 0.3")
{
...
// Create a bitmap-view & add to the window, but only tempory.  Seems daft?  Perhaps, but it keeps things looking good when we open the application...
   ...
   //AddChild(new ScrollBar(Rect(GetBounds().Width() - this->GetBounds().Width(),10,10,10),"sc",NULL,CF_FOLLOW_ALL));
...
// Load requested file
{
  ...
  if ( strstr(cFileName, ".jpg") || (strstr(cFileName,".png")) || (strstr(cFileName, ".gif")) ){
    ...
    if(main_bitmap==NULL){         // We wern't able to load the given file
      float win_width=300;
