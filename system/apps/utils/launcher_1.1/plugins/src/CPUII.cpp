...
#define PLUGIN_NAME    "Launcher CPU Meter"
#define PLUGIN_VERSION "1.0"
#define PLUGIN_AUTHOR  "Henrik Isaksson"
...
class CPUMeter : public GradientBar
{
	public:
		CPUMeter( int nCpuId );
		~CPUMeter();
...
int init( LauncherMessage *pcPrefs )
{
    ...
    string zName = (string)"LauncherCPUII_" + (string)zID;
...
CPUMeter::CPUMeter( int nCpuId ) : GradientBar( Rect(), "CPUMeter" )
{
	m_nCpuId = nCpuId;
	m_vValue = 0;
