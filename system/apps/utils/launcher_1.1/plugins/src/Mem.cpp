...
#define PLUGIN_NAME    "Launcher Mem"
...
int init( LauncherMessage *pcPrefs )
{
    ...
    string zName = (string)"LauncherMem_" + (string)zID;
...
MemMeter::MemMeter( ) : View( Rect(), "MemMeter" )
{
	m_vValue = 0;
}
...
*/
	...
	SetFgColor( 0,0,0 );
	m_vValue = (1.0f - (float(sInfo.nFreePages) / float(sInfo.nMaxPages))) - (float(sInfo.nBlockCacheSize) / float(sInfo.nMaxPages * PAGE_SIZE));
	FillRect( Rect( cBounds.left, cBounds.top, cBounds.left + (m_vValue * cBounds.Width()), cBounds.bottom ) );	
...
Point MemMeter::GetPreferredSize( bool bLargest ) const
{
	return Point( 94, 18 );
}
