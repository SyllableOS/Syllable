	...
	switch (pcMessage->GetCode())
	  { 
		...
		case ID_PEN_CHANGED:
			...
			{
				case 0: pcPropTab->pcColor->pcColorEdit->SetValue(c_bgColor); break;
				case 1: pcPropTab->pcColor->pcColorEdit->SetValue(c_fgColor); break;
			}
...
void PropWin::SaveLoginConfig(bool b_Login, const char* zName)
{
    ...
    if (b_Login == true){fprintf(fin,"true\n\n");}
    else{fprintf(fin,"false\n\n");}
...
void PropWin::SavePrefs(bool bShow, bool bLogin, string zPic, int32 nNewImageSize )
{
	switch(pcPropTab->pcColor->pcColorDrop->GetSelection())
	{
		...
		case 1: c_fgColor = pcPropTab->pcColor->pcColorEdit->GetValue(); break;
	}
