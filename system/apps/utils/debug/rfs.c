...
int server()
{
  ...
  for (;;)
  {
    ...
    int nSize = debug_read( g_nPort, anBuffer, PACKET_SIZE );
	...
	case CMD_OPEN:
	{
	  ...
	  debug_write( g_nPort, (char*)psReply, PACKET_SIZE );
	  bNoWait = true;
	  nPrevPos = 0;
	  break;
	}
	case CMD_CLOSE:
	{
	  ...
	  *((int*)anBuffer) = CMD_CLOSE_REPLY;
	  debug_write( g_nPort, anBuffer, PACKET_SIZE );
	  bNoWait = false;
	  break;
	}
	case CMD_WRITE:
	{
	  ...
	  ((int*)anBuffer)[1] = nCRC;
	  debug_write( g_nPort, anBuffer, 8 );
	  break;
	}
      }
    }
...
//  debug_write( g_nPort, "Hello world\n", PACKET_SIZE );
  ...
  for ( i = 0 ; i < 100 ; ++i )
  {
    ...
    if ( debug_read( g_nPort, anBuffer, PACKET_SIZE ) == PACKET_SIZE )
    {
...
//    printf( "Send %d bytes to remote machine\n", nSize );
    ...
    {
      memset( anBuffer, 0, 8 );
      if ( debug_read( g_nPort, anBuffer, 8 ) == 8 ) {
