...
Color32_s g_cSelectColor( 200, 100, 200 );
...
static Color32_s GetAttrFgColor( int nAttrib )
{
    Color32_s sColor;
...
static Color32_s GetAttrBgColor( int nAttrib )
{
    Color32_s sColor;
...
TermView::TermView( const Rect& cFrame, const char* pzTitle, uint32 nResizeMask, uint32 nFlags )
...
//					g_pcScrollBar->SetMinMax( 0, cNewSize.y );
		}
	    }
...
void TermView::MouseDown( const Point& cPosition, uint32 nButtons )
{
    ...
    m_cSelectStart = cPosition - Point( 0, m_nScrollPos * m_cCharSize.y );
...
void TermView::MouseMove( const Point& cNewPos, int nCode, uint32 nButtons, Message* pcData )
{
	...
	if ( g_bIBeamHalo ) {
	    Application::GetInstance()->PushCursor( MPTR_MONO, g_anMouseImg2, POINTER2_WIDTH, POINTER2_HEIGHT,
						    IPoint( POINTER2_WIDTH / 2, POINTER2_HEIGHT / 2 ) );
	} else {
	    Application::GetInstance()->PushCursor( MPTR_MONO, g_anMouseImg1, POINTER1_WIDTH, POINTER1_HEIGHT,
						    IPoint( POINTER1_WIDTH / 2, POINTER1_HEIGHT / 2 ) );
	}
...
void TermView::RenderCursor( int nAttribs )
{
    ...
    DrawLine( Point( m_cCsrPos.x * m_cCharSize.x, y * m_cCharSize.y + 1 ),
	      Point( m_cCsrPos.x * m_cCharSize.x, y * m_cCharSize.y + m_cCharSize.y -1 )  );
	
    DrawLine( Point( m_cCsrPos.x * m_cCharSize.x + 1, y * m_cCharSize.y + 1 ),
	      Point( m_cCsrPos.x * m_cCharSize.x + 1, y * m_cCharSize.y + m_cCharSize.y -1 ) );
}
...
//		int	 nCharOffset =  x + y * m_cMaxCharMapSize.x;
	...
	if ( bStateChange ) {
		...
		SetFgColor( (nLastAttrib & ATTR_INVERSE) ?
			    GetAttrFgColor( nLastAttrib ):
			    GetAttrBgColor( nLastAttrib ) );
	    } else {
		SetFgColor( g_cSelectColor );
	    }

	    FillRect( Rect( nStartX * m_cCharSize.x, y * m_cCharSize.y,
			    (x + 1) * m_cCharSize.x - 1,
...
//				nSpaceCnt++;;
	...
	if ( bDoRender )
	{
		...
		if ( (nLastAttrib & ATTR_SELECTED) == 0 ) {
		    ...
		    SetBgColor( (nLastAttrib & ATTR_INVERSE) ?
				GetAttrFgColor( nLastAttrib ) :
				GetAttrBgColor( nLastAttrib ) );
		} else {
		    SetFgColor( 0, 0, 0 );
		    SetBgColor( 200, 100, 200 );
		}
		...
		if ( nLastAttrib & ATTR_UNDERLINE ) {
		    DrawLine( Point( nStartX * m_cCharSize.x, (y + 1) * m_cCharSize.y - 1 ),
			      Point( (nStartX + x - nStartX + 1 ) * m_cCharSize.x, (y + 1) * m_cCharSize.y - 1 ) );
		}
	    }
	    x += nSpaceCnt;
	    nSpaceCnt = 0;
...
void	TermView::RefreshDisplay( bool bAll )
{
	...
	if ( NULL != pcFont )
	{
		...
		if ( bAll || m_pabModifiedLines[y] ) {
		    RenderLine( y, m_sFontHeight.ascender, !bAll );
...
void TermView::KeyDown( const char* pzString, const char* pzRawString, uint32 nQualifiers )
{
    ...
    if ( nQualifiers & QUAL_LALT && '\0' != pzString[0] ) {
	if ( pzRawString[0] == 'c' ) {
	    Copy();
	    return;
	} else if ( pzRawString[0] == 'v' ) {
	    Paste();
	    return;
	}
	write( g_nMasterPTY, "\x1b", 1 );
	nChar = pzRawString[0];
    } else {
