...
/*
int syslog(int type, char msg, int len )
{
  return( 0 );
}
*/
...
#define	INTERNALLOG	LOG_ERR|LOG_CONS|LOG_PERROR|LOG_PID
	...
	vfprintf (f, fmt, ap);
	  ...
	  // In Syllable we don't currently use a daemon, but write it directly
	  // to a file so we must add a newline to the buffer.
...
openlog_internal(const char *ident, int logstat, int logfac)
{
  ...
  if ( LogFile == -1 ) {
    LogFile = open( "/var/log/syslog", O_WRONLY | O_APPEND | O_CREAT );
    if ( LogFile >= 0 ) {
      connected = 1;
    }
  }
