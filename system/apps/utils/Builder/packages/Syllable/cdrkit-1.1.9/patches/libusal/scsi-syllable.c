...
LOCAL	char	_usal_trans_version[] = "scsi-syllable.c-1.0";	/* The version for this transport*/
	...
	char	*device;
{
	...
	if (device == NULL || *device == '\0') {
		errno = EINVAL;
		if (scgp->errstr)
			snprintf(scgp->errstr, SCSI_ERRSTR_SIZE,
...
LOCAL void *
usalo_getbuf(scgp, amt)
	SCSI	*scgp;
	long	amt;
{
	if (scgp->debug > 0) {
		fprintf((FILE *)scgp->errfile,
...
LOCAL int
usalo_fileno(scgp, busno, tgt, tlun)
	SCSI	*scgp;
	int	busno;
	int	tgt;
	int	tlun;
	...
	SCSI	*usalp;
{
	...
	if (usalp->debug > 0) {
		error("SEND(%d): cmd %02x, cdb = %d, data = %d, sense = %d\n",
			usalp->fd, cmd.nCommand[0], cmd.nCommandLength,
			cmd.nDataLength, cmd.nSenseLength);
	}
	ret = ioctl(usalp->fd, CD_PACKET_COMMAND, &cmd, sizeof (cmd));
	if( ret < 0 )
