/* Definitions for Intel 386 running Syllable.
...
/* A lie, I guess, but the general idea behind AtheOS is that we are
...
#define LIB_SPEC \
  "%{kernel:-lkernel} %{!kernel:%{shared: -lc}    %{!shared: %{mieee-fp:-lieee} %{pthread:-lpthread} \
   %{profile:-lc_p -lgcc} %{!profile: -lc}}}"


#undef  LIBGCC_SPEC
#define LIBGCC_SPEC "%{!kernel:-lgcc} %{kernel:-lgcc_static }"
...
#define CPP_PREDEFINES "-D__ELF__ -Dunix -D__ATHEOS__ -D__SYLLABLE__ -D_REENTRANT -Asystem(posix)"
...
/* Provide a LINK_SPEC appropriate for AtheOS.  Here we provide support
...
#undef	LINK_SPEC
#define LINK_SPEC "-m elf_i386_atheos %{!r:-shared} %{!plugin:-Bsymbolic --no-undefined} %{kernel:-shared} -L/system/libs/ \
