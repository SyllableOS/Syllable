...
static struct option const long_opts[] = {
	...
	{"direct", no_argument, &g_nDirect, 1},
	{NULL, 0, NULL, 0}
};

void usage( const char *pzStr, int nLevel )
{
	printf( "Usage: %s [device] mountpnt [-hVvrwdto]\n", pzStr );
	if( nLevel > 0 )
	{
		...
		printf( "  -d --direct         mount filesystem without caching\n" );
...
int main( int argc, char **argv )
{
	...
	while( ( c = getopt_long( argc, argv, "hvrwda:t:o:", long_opts, ( int * )0 ) ) != EOF )
	{
		switch ( c )
		{
		case 0:
