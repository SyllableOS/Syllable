#include "extract.h"
ExtractWindow::ExtractWindow(Window* pcParent): Window(Rect(0,0,200,75),"extract_win","Extract To:", WND_NO_ZOOM_BUT | WND_NO_DEPTH_BUT | WND_NOT_MOVEABLE | WND_NOT_RESIZABLE | WID_TRANSPARENT)
{
...
void ExtractWindow::HandleMessage(Message* pcMessage)
{
    ...
    case ID_BROWSE:
        {
            m_pcBrowse = new FileRequester(FileRequester::LOAD_REQ, new Messenger(this),getenv("$HOME"),FileRequester::NODE_DIR,false,NULL,NULL,true,true,"Select","Cancel");
    ...
    case M_LOAD_REQUESTED:
        {
            ...
            // string sPath = pzAPath[strlen(sPath.find( '/').c_str()];
             //m_pcExtractView->Set(sPath.c_str());
    ...
    case ID_GOING_TO_EXTRACT:  {
            ...
            pcMessage->FindString("pzPath",&pzExtractPath);
...
void ExtractWindow::mExtract()
{
    ...
    if((strstr(pzPath,".tar")) && (!strstr(pzPath,".gz")) && (!strstr(pzPath,".bz2")))
    {
...

        {
            ...
            dup2(anPipe[2],2);
            close (anPipe[0] );
            execlp("tar","tar", "-xf",pzPath,"-C",cBuf.c_str(),NULL);
    ...
    if(strstr(pzPath,".bz2")){
...

        {
            ...
            dup2(anPipe[2],2);
            close (anPipe[0] );
            execlp("tar","tar","--use-compress-program=bunzip2 -xf",pzPath,"-C",cBuf.c_str(),NULL);
    ...
    if(strstr(pzPath,".gz")) {
...

        {
            ...
            dup2(anPipe[2],2);
            close (anPipe[0] );
            execlp("tar","tar", "-xvpzf",pzPath,"-C",cBuf.c_str(),NULL);
