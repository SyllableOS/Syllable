...
struct PNode
{
	...
	int m_nStart;
	int m_nEnd;
	bool operator<( const PNode & cNode ) const
	{
		return ( m_nStart < cNode.m_nStart );
	}
};
...
{
      public:
	bool operator(  ) ( const PartitionEntry * p1, const PartitionEntry * p2 )const
	{
		return ( p1->m_nStart < p2->m_nStart );
	}
};
...
static void load_logical( PartitionList_t *pcList, MountMap_t * pcMountMap, const DiskInfo & cDisk, off_t nOffset, off_t nRealOff = 0 )
{
	...
	for( int i = 0; i < 4; ++i )
	{
		...
		else
		{
			...
			pcEntry->m_nEnd = ( sBootBlock.bb_asPartitions[i].p_nStart + nRealOff ) / cDisk.m_sDriveInfo.sectors_per_track + sBootBlock.bb_asPartitions[i].p_nSize / cDisk.m_sDriveInfo.sectors_per_track - 1;
...
PartitionEditReq::PartitionEditReq( const DiskInfo & cDisk ):Window( Rect( 0, 0, 0, 0 ), "partition_edit", MSG_PARTWND_TITLE )
{
	...
	for( int i = 0; i < 4; ++i )
	{
		...
		pcEntry->m_nEnd = m_pcFirstPTable->m_sBootBlock.bb_asPartitions[i].p_nStart / m_cDiskInfo.m_sDriveInfo.sectors_per_track + m_pcFirstPTable->m_sBootBlock.bb_asPartitions[i].p_nSize / m_cDiskInfo.m_sDriveInfo.sectors_per_track - 1;
	...
	case ID_PARTITION_EXTENDED:
		{
			...
			{
				for( PartitionList_t::iterator i = m_pcFirstPTable->m_cLogicalPartitions.begin(  ); i != m_pcFirstPTable->m_cLogicalPartitions.end(  ); )
				{
					if( ( *i )->m_nStart >= nSize )
					{
						delete *i;
			...
			if( m_cEditViews.size(  ) == 1 )
			{
				if( bDoSave && ( m_bTypeChanged || m_bLayoutChanged || m_bStatusChanged ) )
				{
					...
					for( uint i = 0; i < m_cPrimary.size(  ); ++i )
					{
						...
						{
							for( PartitionList_t::iterator j = m_pcFirstPTable->m_cLogicalPartitions.begin(  ); j != m_pcFirstPTable->m_cLogicalPartitions.end(  ); )
							{
								if( ( *j )->m_nStart >= nSize )
								{
									delete *j;
		...
		for( uint i = 0; i < m_cPrimary.size(  ); ++i )
		{
			...
			else
			{
				...
				if( m_cPrimary[i]->IsModified(  ) )
				{
					...
					lba_to_csh( nStart, m_cDiskInfo.m_sDriveInfo, &m_pcFirstPTable->m_sBootBlock.bb_asPartitions[i].p_nStartCL, &m_pcFirstPTable->m_sBootBlock.bb_asPartitions[i].p_nStartCH, &m_pcFirstPTable->m_sBootBlock.bb_asPartitions[i].p_nStartDH );

					lba_to_csh( nEnd, m_cDiskInfo.m_sDriveInfo, &m_pcFirstPTable->m_sBootBlock.bb_asPartitions[i].p_nEndCL, &m_pcFirstPTable->m_sBootBlock.bb_asPartitions[i].p_nEndCH, &m_pcFirstPTable->m_sBootBlock.bb_asPartitions[i].p_nEndDH );
	...
	if( m_bIsExtendedChanged )
	{
		...
		else
		{
			for( uint i = 0; i < m_pcFirstPTable->m_cLogicalPartitions.size(  ); ++i )
			{
				...
				lba_to_csh( nStart, m_cDiskInfo.m_sDriveInfo, &sBootBlock.bb_asPartitions[0].p_nStartCL, &sBootBlock.bb_asPartitions[0].p_nStartCH, &sBootBlock.bb_asPartitions[0].p_nStartDH );

				lba_to_csh( nEnd, m_cDiskInfo.m_sDriveInfo, &sBootBlock.bb_asPartitions[0].p_nEndCL, &sBootBlock.bb_asPartitions[0].p_nEndCH, &sBootBlock.bb_asPartitions[0].p_nEndDH );
				...
				if( i < m_pcFirstPTable->m_cLogicalPartitions.size(  ) - 1 )
				{
					...
					lba_to_csh( nStart, m_cDiskInfo.m_sDriveInfo, &sBootBlock.bb_asPartitions[1].p_nStartCL, &sBootBlock.bb_asPartitions[1].p_nStartCH, &sBootBlock.bb_asPartitions[1].p_nStartDH );

					lba_to_csh( nEnd, m_cDiskInfo.m_sDriveInfo, &sBootBlock.bb_asPartitions[1].p_nEndCL, &sBootBlock.bb_asPartitions[1].p_nEndCH, &sBootBlock.bb_asPartitions[1].p_nEndDH );
...
PartitionEdit::PartitionEdit( const os::Rect & cFrame, const std::string & cName, DiskInfo cDisk, PartitionList_t *pcPList, off_t nDiskSize, bool bIsExtended ):LayoutView( cFrame, cName )
{
	...
	{
		sprintf( zString, MSG_PARTWND_DISKPARAM_DISKSIZE.c_str(  ), double ( m_nPartitionSize * m_sDiskGeom.sectors_per_track * m_sDiskGeom.bytes_per_sector ) / ( 1024.0 * 1024.0 ) );
	}
	else
	{
		sprintf( zString, MSG_PARTWND_DISKPARAM_EXTSIZE.c_str(  ), double ( m_nPartitionSize * m_sDiskGeom.sectors_per_track * m_sDiskGeom.bytes_per_sector ) / ( 1024.0 * 1024.0 ) );
	}
	m_pcDiskSizeLabel = new StringView( Rect(  ), "disk_size", zString );

	sprintf( zString, MSG_PARTWND_DISKPARAM_UNITSIZE.c_str(  ), m_sDiskGeom.sectors_per_track, m_sDiskGeom.bytes_per_sector, m_sDiskGeom.sectors_per_track * m_sDiskGeom.bytes_per_sector );
	...
	if( m_bIsExtended )
	{
		...
		m_pcPrevBut->SetTabOrder( NEXT_TAB_ORDER );
		m_pcNextBut->SetTabOrder( NEXT_TAB_ORDER );
		m_pcAddBut->SetTabOrder( NEXT_TAB_ORDER );
	}
	pcButtonPanel->AddChild( new HLayoutSpacer( "space" ) );
	if( m_bIsExtended == false )
	{
		pcButtonPanel->AddChild( m_pcPartitionBut );
		m_pcPartitionBut->SetTabOrder( NEXT_TAB_ORDER );
	}
	else
	{
		pcButtonPanel->AddChild( m_pcSortBut )->SetBorders( Rect( 0.0f, 10.0f, 10.0f, 10.0f ) );
		m_pcSortBut->SetTabOrder( NEXT_TAB_ORDER );
	}
	pcButtonPanel->AddChild( m_pcClearBut );
	m_pcClearBut->SetTabOrder( NEXT_TAB_ORDER );
	pcButtonPanel->AddChild( m_pcOkBut );
	m_pcOkBut->SetTabOrder( NEXT_TAB_ORDER );
	if( m_bIsExtended == false )
	{
		pcButtonPanel->AddChild( m_pcCancelBut );
		m_pcCancelBut->SetTabOrder( NEXT_TAB_ORDER );
	}
...
void PartitionEdit::Activated( bool bIsActive )
{
	if( bIsActive )
	{
		if( !m_cPartitionViews.empty() )
		{
			m_cPartitionViews.front()->MakeFocus( true );
		}
		else
		{
			m_pcOkBut->MakeFocus( true );
		}
	}
}
...
PartitionView::PartitionView( PartitionEdit * pcParent, const std::string & cName, off_t nDiskSize, int nCylSize, int nSectorSize ):FrameView( Rect(  ), cName, MSG_PARTWND_PARTINFO_UNUSED, CF_FOLLOW_NONE )
{
	m_pcPInfo = NULL;
...
{
	if( bIsActive ) m_pcStartSpinner->MakeFocus( true );
}
...
void PartitionView::SetSpinnerLimits(  )
{
	...
	for( uint i = 0; i < m_pcParent->m_pcPList->size(  ); ++i )
	{
		...
		{
			if( i == j || ( *m_pcParent->m_pcPList )[i]->m_nType == 0 )
				continue;
...
void PartitionView::SliderStartChanged( float *pvStart )
{
	...
	for( uint i = 0; i < m_pcParent->m_pcPList->size(  ); ++i )
	{
		if( i == nIndex || ( *m_pcParent->m_pcPList )[i]->m_nType == 0 )
			continue;
		...
		if( m_pcPInfo->m_nStart - nSpacing <= ( *m_pcParent->m_pcPList )[i]->m_nEnd && m_pcPInfo->m_nEnd >= ( *m_pcParent->m_pcPList )[i]->m_nStart - nSpacing )
		{
			if( m_pcPInfo->m_nStart - nSpacing <= ( *m_pcParent->m_pcPList )[i]->m_nEnd )
			{
				if( m_pcPInfo->m_nStart < ( *m_pcParent->m_pcPList )[i]->m_nStart )
				{
					...
					for( uint j = 0; j < m_pcParent->m_pcPList->size(  ); ++j )
					{
						if( j == nIndex || j == i || ( *m_pcParent->m_pcPList )[j]->m_nType == 0 )
							continue;
						if( nNewEnd >= ( *m_pcParent->m_pcPList )[j]->m_nStart - nSpacing && nNewEnd <= ( *m_pcParent->m_pcPList )[j]->m_nEnd )
						{
							bCollide = true;
							break;
						}
			...
			if( m_pcPInfo->m_nEnd >= ( *m_pcParent->m_pcPList )[i]->m_nStart - nSpacing )
			{
				if( m_pcPInfo->m_nEnd > ( *m_pcParent->m_pcPList )[i]->m_nEnd )
				{
					...
					for( uint j = 0; j < m_pcParent->m_pcPList->size(  ); ++j )
					{
						if( j == nIndex || j == i || ( *m_pcParent->m_pcPList )[j]->m_nType == 0 )
							continue;
						if( nNewStart >= ( *m_pcParent->m_pcPList )[j]->m_nStart - nSpacing && nNewStart <= ( *m_pcParent->m_pcPList )[j]->m_nEnd )
						{
							bCollide = true;
							break;
						}
...
PartitionBar::PartitionBar( const Rect & cFrame, PartitionView * pcParent, const std::string & cName, float vStart, float vEnd ):View( cFrame, cName, CF_FOLLOW_NONE, WID_FULL_UPDATE_ON_RESIZE )
{
	m_vStart = vStart;
	m_vEnd = vEnd;
	m_vDragOffset = 0.0f;
...
void PartitionBar::MouseMove( const Point & cNewPos, int nCode, uint32 nButtons, Message * pcData )
{
	...
	if( nCode != MOUSE_EXITED && cBounds.DoIntersect( cNewPos ) && ( fabs( cNewPos.x - vStart ) < 4.0f || fabs( cNewPos.x - vEnd ) < 4.0f ) )
	{
		...
		{
			Application::GetInstance(  )->PushCursor( MPTR_MONO, g_anHMouseImg, H_POINTER_WIDTH, H_POINTER_HEIGHT, IPoint( H_POINTER_WIDTH / 2, H_POINTER_HEIGHT / 2 ) );
			m_bDragPointer = true;
		}
	}
