...
/* This refers to the current colormap.  This is *NOT* the same as the list
 * of colors; it is an data structure used internally by Gdk.
 */
//GdkRgbCmap *color_map = NULL; 
os::Color32_s color_map[256];
 ...
 */
static guint32 (*stylefunc)(guint32);
...
static guint32 tored, togreen, toblue;
static gboolean tonew;
static guint32 fromred, fromgreen, fromblue;
static guint32 bgred, bggreen, bgblue;
...
static guint32 dimming(guint32 i)
{
...
static guint32 brightening(guint32 i)
{
	i = 255 - i;
...
static guint32 milky(guint32 i)
{
	guint32	r, g, b, tmp, k;
	if (i < 128)
	{
...
static guint32 cloud(guint32 i)
{
...
static guint32 metal(guint32 i)
{
	guint32 r, g, b, k;
...
static guint32 layers(guint32 i)
{
	guint32	k;
...
static guint32 colorlayers(guint32 i)
{
	guint32 tmp, r, g, b, k;
...
static guint32 colorstandoff(guint32 i)
{
	guint32 tmp, r, g, b, k;
...
static guint32 flame(guint32 i)
{
	hsv_t	hsv;
	guint32 k;
...
static guint32 rainbow(guint32 i)
{
	hsv_t	hsv;
	guint32 k;
...
static guint32 standoff(guint32 i)
{
	guint k;
...
static guint32 threshold(guint32 i)
{
...
static guint32 stripes(guint32 i)
{
	guint32	tmp, k;
...
static guint32 colorstripes(guint32 i)
{
	guint32	r, g, b, k, tmp;
...
static guint32 colorbands(guint32 i)
{
	guint32	r, g, b, k, tmp;
...
static guint32 graying(guint32 i)
{
	guint32 faded, tmp;
...
static guint32 noise(guint32 i)
{
	if (rand_0_to(256) < i)
		return config.color;
	else
 ...
 */
void color_genmap(int do_random)
{
	gint32 i;

	/* Decompose the dominant color into R/G/B components */
	red = (guint32)(config.color / 0x10000);
	green = (guint32)((config.color % 0x10000)/0x100);
	blue = (guint32)(config.color % 0x100);
 ...
 */
void color_bg(gint ndata, gint16 *data)
{
	guint32	bgr, bgb, bgg, k, bg;
	guint32	fgr, fgb, fgg;
	int	i, j;
	gint16	max, min;
	gint32	totdelta;
	guint32 newcolors[256];
	static guint32 fallr, fallg, fallb;
	...
	else /* "Flash bkgnd" */
	{
		...
		else
		{
			 ...
			 * been conditioned to fill the gint16 range.
			 */
	...
	for( i = 0; i < 256; i++ )
	{
		color_map[i] = os::Color32_s( newcolors[i] );
	}
	
#if 0
	if (color_map)
	{
		gdk_rgb_cmap_free(color_map);
	}
	color_map = gdk_rgb_cmap_new(newcolors,256);
#endif
}

/* Discard the color_map when blursk shuts down */
...
void color_beat(void)
{
	...
	config_load_color(color);

	/* That's all!  The config window will call color_genmap() to produce
	 * a new colormap using the new color, automatically.
	 */
}
