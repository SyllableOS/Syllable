...
static gint16	renderdata[MAXPOINTS];


/* Draw a line between two points, in a given color */
static void line(gint x, gint y, gint x2, gint y2, guchar color)
{
	gint	xdiff, ydiff;
	gint	error;
	int	bpl;
	register guchar	*point;
	register int i;
...
static void fuzzydot(gint x, gint y, gint add)
{
	int	xx, yy;
	int	sum;
	guchar	*point;
...
static void plussign(gint x, gint y, gint add)
{
	int	extent, i;
	guchar	*point;
...
void render_dot(gint x, gint y, guchar color)
{
	int x2, y2;
...
static void airbrush(gint x, gint y, guchar color)
{
...
static void edges(gint x, gint y, gint thick)
{
...
static gint plotfirst;
static gint plotthick;
static guchar plotcolor;
static double plottheta = 0.0;
static double plotsin, plotcos;
static double plotprevsin, plotprevcos;
static gint plotcount;
static gint plotmax;
static gint plotx[512], ploty[512];
static gint plotprevmax;
static gint plotprevx[512], plotprevy[512];

static void plotbefore(gint thick, gint max)
{
	plotthick = thick;
	plotfirst = TRUE;
	plotcount = 0;
...
static void plot(gint x, gint y)
{
	static gint fromx, fromy;
	int	i;
 ...
 */
gint condition_data(gint max, gint ndata, gint16 **dataref)
{
	int	i, j, step;
	...
	while (ndata < max/2)
	{
		...
		*dataref = renderdata;
		...
		/* smooth */
		for (i = 1; i < ndata - 2; i++)
			renderdata[i] = ((renderdata[i - 1] + renderdata[i + 1]) * 3 + renderdata[i] * 10) >> 4;
	}
	return ndata;
}

/* Plot a radial spectrum. */
static void render_radial(gint thick, gint center, gint ndata, gint16 *data)
{
	double	theta, step, sig;
	int	i;
	gint	x, y, xscale, xcenter, yscale;
	static gint16 prev[MAXPOINTS];
	static gint nprev;
	gint32	trigger;
	...
	{
		for (i = 0; i < ndata; i++)
			prev[i] = ((gint32)prev[i] + (gint32)data[i]) >> 1;
		data = prev;
	}
	else
	{
		nprev = ndata;
		memcpy(prev, data, ndata * sizeof(gint16));
	}
#endif
...
static void render_phase(gint thick, gint center, gint ndata, gint16 *data)
{
	...
	for (i = 0; i < npoints; i++)
	{
		...
		x = (data[sample] >> 8) + img_width/2;
		y = (data[sample + ndata/8 - 1] >> 9) + center;
...
static void render_highlow(gint thick, gint center, gint ndata, gint16 *data)
{
	gint16	*d1, *d2;
	gint32	trigger;
	gint	i, x, y;
	...
	for (i = 0; i < ndata; i++)
	{
		...
		if (trigger >= BEAD_THRESHOLD)
		{
			 ...
			 * the samples.
			 */
			x = img_width / 2 + (((i - ndata/2) * (gint32)img_width * thick) >> 7);
			y = center + (((gint32)d1[i] - (gint32)d2[i]) >> 8);
			plot(x, y);
		}
	}
	plotafter();
 ...
 */
static void render_curve(gint thick, gint center, gint ndata, gint16 *data)
{
	gint i, x, y;
	gint32 scale;
	...
	for(i = 0; i < ndata; i++)
	{
		...
		y = center + (((gint32)data[i] * scale) >> 14);
		if(y < 0)
			y = 0;
		if(y >= img_height - 1)
			y = img_height - 2;
...
void render(gint thick, gint center, gint ndata, gint16 *data)
{
